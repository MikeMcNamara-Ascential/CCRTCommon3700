...
BOOL clientStart(void)
{
    ...
    gpMemoryPool = gMemoryPool;        /* Mark memory pool as empty */
#endif    
	...
	if(ghClientMutex == NULL)
	{
		...
		notifyEvent( NM_UNABLE_START_THREAD, 0, 0 );
		return FALSE;
	}
...
{    
    ...
    for( pSession= gSessions; pSession < gpnSessions; pSession++ )
    {
        ...
        {                
            ucmmIssueUnRegisterSession( pSession );    /* Send Unregister Session request to the server */                
        }    
    }
#endif
...
{    
    ...
    if ( ( (lTickIncrement % TCP_MINIMUM_RATE) == 0 ) || ( nConnProcessedTotal < CONN_PROCESSED_LIMIT ) )
    {
...
INT32 clientGetConnectionStats(INT32 nConnectionInstance, EtIPConnectionStats* pStats, BOOL bClear)
{
    ...
    pStats->lRcvdPackets = pConnection->lRcvdPackets;    
    pStats->lSendPackets = pConnection->lSendPackets;
	...
	pStats->lRcvdBytes = pConnection->lRcvdBytes;
	pStats->lSendBytes = pConnection->lSendBytes;
...
INT32 clientSendConnectionResponse(INT32 nConnectionInstance, EtIPErrorInfo* pErrorInfo, UINT8 bAppSize, UINT8* pAppData)
{
	...
	if (pErrorInfo->bGeneralStatus == 0)
	{
		...
		else
		{
			...
			connmgrPrepareConnection(NULL, pConnection, pRequest, bAppSize, pAppData);
#ifdef ET_IP_MODBUS
		}
#endif
	}
...
INT32 clientStartRequestGroup(void)
{
	...
	if ( pGroup == NULL )                        /* No more space for request groups */
    {
...
INT32 clientSendUnconnectedRequest( EtIPObjectRequest* pRequest )
{
    ...
    if( pRequest->lHostIPAddr == INADDR_NONE )
        pRequest->lHostIPAddr = gsTcpIpInstAttr[0].InterfaceConfig.lIpAddr;
    
    platformWaitMutex(ghClientMutex, MUTEX_TIMEOUT);

    if ( bLocalRequest )    
        nRequest = requestNewUnconnectedSend( (unsigned char *)szNetworkPath, pRequest, NULL, 0 );    /* Schedule UCMM */
    else
        nRequest = requestNewUnconnectedSend( (unsigned char *)szNetworkPath, pRequest, szExtendedPath, iExtendedPathLen );/* Schedule Unconnected Send */
...
INT32 clientSendEPATHUnconnectedRqst( EtIPEPATHObjectRequest* pEPATHRequest )
{
    ...
    /* If the user did not specify Request Host IPAddress - use the first one */
    if( pEPATHRequest->lHostIPAddr == INADDR_NONE )
        pEPATHRequest->lHostIPAddr = gsTcpIpInstAttr[0].InterfaceConfig.lIpAddr;
    
    platformWaitMutex(ghClientMutex, MUTEX_TIMEOUT);
    
    if ( bLocalRequest )    
        nRequest = requestNewEPATHUnconnectedSend( (unsigned char *)szNetworkPath, pEPATHRequest, NULL, 0 );    /* Schedule UCMM */
    else
        nRequest = requestNewEPATHUnconnectedSend( (unsigned char *)szNetworkPath, pEPATHRequest, szExtendedPath, iExtendedPathLen );/* Schedule Unconnected Send */
...
INT32 clientGetClientRequest( INT32 nRequestId, EtIPObjectRequest* pObjectRequest )
{
	...
	{
		pduInit( &pdu, TRUE );
		pduParse(pRequest->pExtendedPath, &pdu, TRUE, 0);
    ...
    pData = (INT32)pduGetTagByType(&pRequest->cfg.objectCfg.PDU, TAG_TYPE_EXT_SYMBOL, 0, &iTagSize);    
    if ( iTagSize )
    {
...
INT32 clientClaimHostIPAddress( UINT32 lHostIPAddress, UINT16 iPort )
{
	...
	DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Checking if %d [%X] has been claimed", 
    ...
    {
		DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "IP Address has already been claimed");
        platformReleaseMutex(ghClientMutex);
        return TRUE;
    }

	DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Checking if %d [%X] is in the host address list", 
			 lHostIPAddress, lHostIPAddress);
    ...
    {
		DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "IP Address is in the host address list");
        platformReleaseMutex(ghClientMutex);
        return FALSE;
    }

	DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Adding claimed IP Address");
...
void clientClaimAllHostIPAddress(void)
{
    UINT32 i;
    UINT32* pIP;
	DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "clientClaimAllHostIPAddress() - IP Addr count: %d", gnHostIPAddrCount);
    for ( i = 0, pIP = galHostIPAddr;
          i < gnHostIPAddrCount; 
          i++, pIP++)
    {
		DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Attempting to claim: %d [%X]", *pIP,*pIP);
...
INT32 clientGetListIdentityResponse(INT32 nId, EtIPListIdentityInfo* pInfo)
{
    ...
    {
        platformReleaseMutex(ghClientMutex);
        return ERR_OBJECT_REQUEST_UNKNOWN_INDEX;        /* Id was not found */
    }
...
EXTERN INT32 clientGetSystemStats(EtIPSystemStats* pStats, BOOL bClear)
{
	...
	memcpy(pStats, &gSystemStats, sizeof(EtIPSystemStats));
...
#ifdef CONNECTION_STATS
	platformWaitMutex(ghClientMutex, MUTEX_TIMEOUT);
	memcpy(&gSystemStats, pStats, sizeof(EtIPSystemStats));
	platformReleaseMutex(ghClientMutex);
	return 0;
#else
	return -1;
