...
UINT16  connmgrAttributes[CONNMGR_ATTR_OPT_COUNT] = {CONNMGR_ATTR_OPEN_REQUESTS, CONNMGR_ATTR_OPEN_FORMAT_REJECTS, CONNMGR_ATTR_OPEN_RESOURCE_REJECTS, CONNMGR_ATTR_OPEN_OTHER_REJECTS, CONNMGR_ATTR_CLOSE_REQUEST, CONNMGR_ATTR_CLOSE_FORMAT_REJECTS, CONNMGR_ATTR_CLOSE_OTHER_REJECTS, CONNMGR_ATTR_CONNECTION_TIMEOUTS};


/* local functions */
void connmgrParseClassRequest( REQUEST* pRequest );
BOOL connmgrParseInstanceRequest( REQUEST* pRequest, UINT32 iInstance );
void connmgrSendClassAttrAll( REQUEST* pRequest );
void connmgrSendClassAttrSingle( REQUEST* pRequest);
void connmgrSendInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance );
void connmgrSendInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
void connmgrSetInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance );
void connmgrSetInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );

BOOL connmgrIncomingConnection( REQUEST* pRequest, BOOL bLargeFwdOpen );
BOOL connmgrNULLConnection(SESSION* pSession, REQUEST* pRequest, CONNECTION* pConnection, FWD_OPEN* pFwdOpen);
void connmgrProcessFwdClose( REQUEST* pRequest );
void connmgrPrepareFwdCloseReply( CONNECTION* pConnection, REQUEST* pRequest );
void connmgrPrepareFwdCloseErrorReply(REQUEST* pRequest, FWD_CLOSE* pFwdClose, UINT8 bGeneralError, UINT16 iExtendedError);
void connmgrProcessUnconnectedSend( REQUEST* pRequest );
void connmgrGetConnectionOwner( REQUEST* pRequest );

BOOL connmgrVerifyRedundantOwnerRequest(CONNECTION *pNewConn, CONNECTION *pExistConn);
...
BOOL  connmgrIncomingConnection( REQUEST* pRequest, BOOL bLargeFwdOpen )
{
    ...
    INT32       pdu_len;
    PDU_HDR     pdu_hdr;
    UINT16      iTagSize, iClassTagSize, iInstanceTagSize, iSymbolTagSize, iFwdOpenSize;
	UINT32      iClass, iInstance, lPort, iSlot = INVALID_SLOT_INDEX;
	ETAG_DATATYPE	pAddr, pSymbol;
#ifdef ET_IP_MODBUS
	UINT32		lIPAddress;
#endif
	...
	if ( pSession == NULL )
    {
        ...
        pRequest->iExtendedError = ROUTER_EXT_ERR_VENDOR_SPECIFIC;
        DumpStr1(TRACE_LEVEL_ERROR, TRACE_TYPE_SESSION, pRequest->lHostIPAddr, pRequest->lIPAddress, 
				"connmgrIncomingConnection INTERNAL ERROR: session with Id %d pulled from under us", pRequest->nSessionId);
        return TRUE;
    }
    ...
    if ( pConnection == NULL )         /* Can not exceed the maximum number of concurent connections limit */
    {
    ...
    nPathSize = pduParse( pPacketPtr, &pConnection->cfg.PDU, TRUE, 0);
    if ( nPathSize == 0 )
    {
		connmgrStats.iOpenFormatRejects++;
	...
	if (iTagSize != 0)
	{
		...
		else
		{
			...
			if ( lIPAddress != 0 )
			{
				...
				return eipmbsParseCIPModbusFORequest(pSession, pRequest, pConnection, &FwdOpen, bLargeFwdOpen);
			}
			else
			{
				connmgrStats.iOpenFormatRejects++;
	...
	pSymbol = pduGetTagByType(&pConnection->cfg.PDU, TAG_TYPE_EXT_SYMBOL, 0, &iSymbolTagSize);
	...
	{
		if ( !(iClassTagSize == 0 && iInstanceTagSize == 0 && iSymbolTagSize > 0 ) )		
		{
			connmgrStats.iOpenOtherRejects++;
			connmgrPrepareFwdOpenErrorReply(pRequest, &FwdOpen, ROUTER_ERROR_FAILURE, ROUTER_EXT_ERR_INVALID_SEGMENT_TYPE_VALUE);
			connectionRemove( pConnection, FALSE );
              ...
              ( pConnection->cfg.bTransportClass == Class3 && iClass != ROUTER_CLASS ) ) 
	{
		...
		{
			connmgrPrepareFwdOpenReply(pSession, pConnection, pRequest, 0, NULL);
			connectionRemove( pConnection, FALSE );
			return TRUE;
		}
    ...
    if (connectionIsIOConnection(pConnection))
    {
        ...
        if ((pConnection->cfg.iTargetConsumingConnPoint != HEARTBEAT_CONN_POINT) &&
            (pConnection->cfg.iTargetConsumingConnPoint != LISTEN_ONLY_CONN_POINT))
        {
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
		...
		if ( pConnection->cfg.iTargetProducingConnPoint != HEARTBEAT_CONN_POINT )            
        {
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
	...
	{
		/* NULL forward open */
		return connmgrNULLConnection(pSession, pRequest, pConnection, &FwdOpen);
	}
    ...
    if (connectionIsIOConnection(pConnection))
	{
		...
		for ( pLoopConnection = gConnections; pLoopConnection < gpnConnections; pLoopConnection++ )
		{
			...
			if ( pLoopConnection->cfg.iTargetProducingConnPoint != 0 && 			 
				 pLoopConnection->cfg.iTargetProducingConnPoint != HEARTBEAT_CONN_POINT && 
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
...
#ifdef ET_IP_SCANNER            
        pConnection->bCCConfigured = FALSE;                /* Forward Open was not preceeded with the CC config request for an incoming connection */            
        ...
        if ( pConnection->cfg.iProducingConnectionType == Multicast )
		{
			...
			{            
				if ( pLoopConnection != pConnection &&
					 ...
					 pLoopConnection->cfg.iTargetProducingConnPoint == pConnection->cfg.iTargetProducingConnPoint &&
					 pLoopConnection->cfg.iTargetConsumingConnPoint != LISTEN_ONLY_CONN_POINT )
				{                
	...
	connmgrPrepareConnection(pSession, pConnection, pRequest, 0, NULL);
    return TRUE;
}
...
**---------------------------------------------------------------------------
*/
void connmgrPrepareConnection(SESSION* pSession, CONNECTION* pConnection, REQUEST* pRequest, UINT8 bAppSize, UINT8* pAppData)
{
	CONNECTION* pLoopConnection, 
				*pOwnerConnection = NULL;
    FWD_OPEN    FwdOpen;
		...
		(pConnection->cfg.iProducingConnectionType == NullConnType))
	{
		...
		else
		{
			...
			connmgrPrepareFwdOpenReply(pSession, pConnection, pRequest, bAppSize, pAppData);
		}
	...
	else
	{
		...
		if (connectionIsIOConnection(pConnection))
		{
			gSystemStats.iNumCurrentIOConnections++;
			if (gSystemStats.iMaxActiveIOConnections < gSystemStats.iNumCurrentIOConnections)
				gSystemStats.iMaxActiveIOConnections = gSystemStats.iNumCurrentIOConnections;
		}
		else if (pConnection->cfg.bTransportClass == Class3)
		{
			gSystemStats.iNumCurrentExplicitConnections++;
			if (gSystemStats.iMaxActiveExplicitConnections < gSystemStats.iNumCurrentExplicitConnections)
				gSystemStats.iMaxActiveExplicitConnections = gSystemStats.iNumCurrentExplicitConnections;
		}
#endif
...
void  connmgrPrepareFwdOpenReply( SESSION* pSession, CONNECTION* pConnection, REQUEST* pRequest, UINT8 bAppSize, UINT8* pAppData)
{
	...
	/* left in if debug statements are needed */
	pSession = pSession;
...
void  connmgrPrepareFwdCloseReply( CONNECTION* pConnection, REQUEST* pRequest )
{
    ...
    if ( pRequest->cfg.objectCfg.iDataSize && pRequest->cfg.objectCfg.pData == NULL )    /* we are out of memory */
    {
...
void connmgrInitBufFromFwdOpenReply( UINT8* pBuffer, FWD_OPEN_REPLY* fwdOpenTypeReply )
{    
...
void connmgrInitFwdCloseTypeFromBuf( UINT8* pBuffer, FWD_CLOSE* fwdCloseType )
{
...
void connmgrInitBufFromFwdCloseReply( UINT8* pBuffer, FWD_CLOSE_REPLY* fwdCloseTypeReply )
{    
...
void connmgrSendClassAttrAll( REQUEST* pRequest )
{
	...
	EtIP_realloc(&pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, (UINT16)((6+iNumberOfAttributes)*sizeof(UINT16)));
...
void connmgrSendInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance )
{
	...
	EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, sizeof(UINT16)*CONNMGR_ATTR_OPT_COUNT);
...
void  connmgrProcessUnconnectedSend( REQUEST* pRequest )
{
    ...
    if ( pNewRequest == NULL)                        /* No more space for requests */
    {
    ...
    nPDUSize = pduParse( pData, &pNewRequest->cfg.objectCfg.PDU, TRUE, 0);
    if ( nPDUSize == 0 )
    {
    ...
    if ( pNewRequest->cfg.objectCfg.iDataSize % 2 )    /* Skip the pad byte if data length is odd */
        pData++;
    ...
    *(pData+1) = (UINT8)(PLATFORM_MIN(nPathSize/2, 2));			/* Make it parse only the first path segment */
    ...
    nPDUSize = pduParse( pData, &pdu, TRUE, 0);
    if ( nPDUSize == 0 )
    {
        requestRemove( pNewRequest );
        pRequest->bGeneralError = ROUTER_ERROR_INVALID_SEG_TYPE;                
...
void connmgrGetConnectionOwner( REQUEST* pRequest )
{
	UINT8	bPathSize,
			bNumConnections = 0,
			bNumCOOConnections = 0,
			bNumROOConnections = 0,
			bLastAction = LAST_ACTION_NO_OWNER;
	UINT16	iPDUParseSize,
			iOwnerConnSerialNumber = 0,
			iOwnerOrigVendorID = 0;
	...
	iPDUParseSize = (UINT16)pduParse(pRequest->cfg.objectCfg.pData, &ConnConfig.PDU, TRUE, 0);
