        ...
        {                    // Read the data from the brake force array
            ...
            {                 // Evaluate the reduction and recovery values
                ...
                {              // Find the actual start and end reduction indices.
                    ...
                    if((testPass != wheelRecovResults[wheelIndex]) || (testPass != wheelReduxResults[wheelIndex]))
                    {//reduction or recovery failed increase wheel failure count
                        wheelFailureCount++;
                    }
                    Log(LOG_DEV_DATA, "%s Reducion: %s, Recovery: %s, WheelFailureCount: %d\n", rollerName[wheelIndex].c_str(),
                        wheelReduxResults[wheelIndex].c_str(), wheelRecovResults[wheelIndex].c_str(), wheelFailureCount);
                }
                // Perform the valve cross check
                BEP_STATUS_TYPE valveCrossStatus;
                //more than one wheel must fail to indicate a valve is crossed
                if(okayToRunAnalyzeValveCross && (wheelFailureCount > 1))
                {
