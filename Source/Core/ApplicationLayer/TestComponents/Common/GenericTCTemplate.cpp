...
const string GenericTCTemplate<ModuleType>::CommandTestStep(const string& value)
{
   ...
   {   // Get the test step name to perform
      ...
      else if (step == "CheckPartNumber" || step == "GenericPartNumberCheck") status = CheckPartNumber();
      // Validate the module part number
...
// Report info read from the module
      ...
      else if (step == "ReadFaultsByPhase") status = ReadFaultsByPhase(value);
...
{   // Set up some variables
   ...
   {   // Need to perform this test step
      ...
      {   // Try to start the keep alive thread
         if (messageId == "") moduleStatus = m_vehicleModule.StartModuleKeepAlive();
         else moduleStatus = m_vehicleModule.StartSpecialKeepAlive(messageId);
   ...
   {   // Do not need to skip this step
      ...
      { // Try to read the module faults
         ...
         {   // Good read, evaluate the data
            ...
            else Log(LOG_DEV_DATA, "Unregistered faults present. Not commandning fault clear.");
         }
