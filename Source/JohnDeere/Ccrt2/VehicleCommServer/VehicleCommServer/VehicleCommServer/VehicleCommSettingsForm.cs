...
namespace VehicleCommServer
{
    public partial class VehicleCommSettingsForm : Form
    {
        ...
        public VehicleCommSettingsForm()
        {
            InitializeComponent();
        ...
        {   // Clear the port combo box
            ...
            foreach (J2534Device device in m_vcsInterface.J2534Devices)
            {
                m_deviceComboBox.Items.Add(device.Name);
            }
        ...
        {   // Clear the channel name combo box
            ...
            foreach (string chName in m_vcsInterface.DbInterface.GetCommChannelNames(m_selectedDevice.Name))
            {
        ...
        {   //select settings for each combo box
            ...
            {
                CcrtJ2534Channel channel = m_vcsInterface.DbInterface.ReadCommChannelSettings(m_deviceComboBox.SelectedItem.ToString(),
        ...
        {
            m_selectedDevice = m_vcsInterface.J2534Devices.Find(delegate(J2534Device dev)
            {
        ...
        private void m_removeDeviceButton_Click(object sender, EventArgs e)
        {
            ...
            {
                m_vcsInterface.DbInterface.RemoveCommChannelSetting(m_deviceComboBox.SelectedItem.ToString(),
                                                    m_commNameComboBox.SelectedItem.ToString());
        ...
        {   // Get all the comm channel setting items from the form
            ...
            {   // Input device already exists, just update the item settings in the data base
                m_vcsInterface.DbInterface.UpdateCommChannelSettings(m_selectedDevice.Name, settings);
            }
            else
            {   // This is a new item, so add the item to the database
                if (commChannelName != "")
                {
                    m_vcsInterface.DbInterface.AddCommChannelSettings(m_selectedDevice.Name, settings);
                    PopulateCommChannelComboBox();
