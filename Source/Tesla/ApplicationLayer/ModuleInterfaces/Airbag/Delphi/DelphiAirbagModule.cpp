...
template <class ProtocolFilterType>
BEP_STATUS_TYPE DelphiAirbagModule<ProtocolFilterType>::ProgramVIN(string vin)
{   // Let the developer know we can't do this.
    BEP_STATUS_TYPE status = BEP_STATUS_FAILURE;
    SerialArgs_t moduleVin;
    Log(LOG_FN_ENTRY, "DelphiAirbagModule::ProgramVIN() - Enter\n");
    if(vin.size() != 0)
    {
        for(int ind=0; ind < vin.size(); ind++)
        {
            moduleVin.push_back((UINT8)vin.at(ind));
        }
        // Program VIN to module
        if ((status=CommandModule("WriteVIN",&moduleVin)) == BEP_STATUS_SUCCESS)
        {
            Log(LOG_DEV_DATA, "Wrote Vehicle VIN to module successfully");
        }
        else
        {
            Log(LOG_ERRORS, "Failed to write Vehicle VIN to the module - status: %s",ConvertStatusToResponse(status).c_str());
        }
    }
    else
    {
        Log(LOG_ERRORS,"VIN is NULL\n");
    }
    Log(LOG_FN_ENTRY, "DelphiAirbagModule::ProgramVIN() - Exit\n");
    return status;
}
