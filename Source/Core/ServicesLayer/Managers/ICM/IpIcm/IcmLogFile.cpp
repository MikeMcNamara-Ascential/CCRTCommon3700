...
void IcmLogFile::Initialize( IcmTagList *tagList, const XmlNode* configNode)
{
    ...
    if( (err=Lock()) == EOK)
    {
        ...
        try
        {
            ...
            if( status == XmlString( XML_T( "Disabled")))   SetEnabled(false);
        ...
        try
        {
            ...
            {
                if( ftpRoot != NULL)    m_logFileName = ftpRoot + XmlToAscii( fileName);
                else                    m_logFileName = XmlToAscii( fileName);
            }
            else
            {
        ...
        try
        {
            ...
            if( ftpRoot != NULL)    m_logFileLink = ftpRoot + XmlToAscii( link->getValue());
            else                    m_logFileLink = XmlToAscii( link->getValue());
        }
        catch( XmlException &err)
        {
...
void IcmLogFile::StartLog()
{
    ...
    if( (err=Lock()) == EOK)
    {
        ...
        if( IsEnabled())
        {
            ...
            if( m_tagList != NULL)  m_tagList->Clear();
...
{
    int retVal;
    if( block)  retVal = m_logFileLock.Acquire();
    else        retVal = m_logFileLock.TryAcquire();
    if(retVal != EOK)   printf("Error Locking The Mutex: %d\n", retVal);
    return( retVal);
}
