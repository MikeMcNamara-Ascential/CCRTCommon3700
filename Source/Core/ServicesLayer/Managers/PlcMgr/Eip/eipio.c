...
{    
    ...
    {        
...
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
        ...
        {
            DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, lOriginatingAddress, 0, 
					 "ioParseIOPacket invalid iS_count");
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
            return nPacketOffset;
        ...
        {
            DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, lOriginatingAddress, 0, 
					 "ioParseIOPacket invalid iAs_type");
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
            return nPacketOffset;
        ...
        {
            DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, lOriginatingAddress, 0, 
					 "ioParseIOPacket invalid iAs_length");
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
            return nPacketOffset;
        ...
        {
            DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, lOriginatingAddress, 0, 
					 "ioParseIOPacket invalid iDs_type");
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
            return nPacketOffset;
		...
		{
            DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, lOriginatingAddress, 0, 
					 "ioParseIOPacket doesn't have complete packet");
#ifdef CONNECTION_STATS
			gSystemStats.iNumFailedRecvIOMessages++;
#endif
            return nPacketOffset;
		...
		do
		{
			...
			for (; pConnection < gpnConnections; pConnection++ ) 
			{
					 ...
					 connectionIsIOConnection(pConnection)) /* If it's a listen only incoming connection disregard the input */
				{
					...
					if (lOriginatingSocket != pConnection->lClass1Socket)
					{
						pConnection = NULL;     /* set to NULL so packet is ignored below */
					}
					break;
				}
			}
			...
			if (pConnection != NULL && pConnection < gpnConnections)
			{
				...
				if ( pConnection->cfg.bInputRunProgramHeader )
				{
					...
					bRunFlag = lRunIdleHeader & IO_RUN_IDLE_MASK;
								...
								if ( bRunFlag )
								{
									...
									if (bNewData)
									{
									   ...
									   if ( assemblyNotifyInstanceChange(pConnection->cfg.iTargetConsumingConnPoint, pDataLocal, (UINT16)iLenLocal, bForceNotify) )
									   {
										  ...
										  assemblySetInstanceData(pConnection->cfg.iTargetConsumingConnPoint, pDataLocal, (UINT16)iLenLocal );    /* Store data in assembly object */
									   }
									   else
									   {
										  /* If data is the same, but seq count changed - notify as required */ 
				...
				connectionResetConsumingTicks( pConnection );        /* Reset timeout ticks */
				...
				{                    
					...
					pConnection->lRcvdBytes += iLen;
				}

				gSystemStats.iNumSuccessfulRecvIOMessages++;
...
INT32  ioSendIOPacket( CONNECTION* pConnection, CONNECTION* pMulticastProducer )
{
		...
		)
    {
        ...
        if ( pConnection->cfg.iTargetProducingConnPoint != HEARTBEAT_CONN_POINT &&
			 pConnection->cfg.iTargetProducingConnPoint != LISTEN_ONLY_CONN_POINT )					
		{
	...
	else if (assemblyIsScannerOutputInstance(pConnection->cfg.iTargetProducingConnPoint))
	{
        ...
        if ( pConnection->cfg.iTargetProducingConnPoint != HEARTBEAT_CONN_POINT &&
			 pConnection->cfg.iTargetProducingConnPoint != LISTEN_ONLY_CONN_POINT )			
			assemblyGetInstanceData(pConnection->cfg.iTargetProducingConnPoint, pData, iDataLen);    /* Get data from assembly object */        
    ...
    if ( pConnection->bCollectingStats )
    {
        pConnection->lSendBytes += iLen;
    }
#endif

    return socketClass1Send( pConnection, (CPFHDR_SIZE + iLen) );                    /* Produce data */    
}
