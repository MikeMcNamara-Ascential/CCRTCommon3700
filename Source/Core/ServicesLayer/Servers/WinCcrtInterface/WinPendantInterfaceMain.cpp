...
int main(int argc, char *argv[])
{
	...
	try
	{
		if(clp.IsDebugOn())	printf("Creating The WinPendantInterface\n");
		server = new WinPendantInterface();		// create a WinCcrtInterface server
		// create, initialize, and process messages for the named data broker
		if(clp.IsDebugOn())	printf("Initializing The WinPendantInterface\n");
		server->Initialize(clp.GetConfigFile());
		if(clp.IsDebugOn())  printf("WinPendantInterface (%d, %s) Up - Listening\nRunning The WinPendantInterface\n",
									BposGetMyTaskId(), clp.GetConfigFile().c_str());
		server->Run();							// process until terminated
	}
	catch(BepException &BepErr)
	{
		printf("WinPendantInterface -%s: BepException: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("WinPendantInterface -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}
	if(clp.IsDebugOn())	printf("Deleting The WinPendantInterface\n");
	if( NULL != server)	delete server;
	if(clp.IsDebugOn())  printf("WinPendantInterface (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
}
