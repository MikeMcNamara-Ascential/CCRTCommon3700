...
//
// FILE DESCRIPTION:
//  Main loop for a IsuzuPlantHostInbound.  
//  
//===========================================================================
...
/**
 * Main loop for the IsuzuPlantHostInbound
 *
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        IsuzuPlantHostInbound server;        // Server object
    ...
    {
        printf("IsuzuPlantHostInbound -%s: Exception: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
    }
    catch(...)
    {
        printf("IsuzuPlantHostInbound -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
    }
    if(clp.IsDebugOn()) printf("IsuzuPlantHostInbound(%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
    return(0);
}
