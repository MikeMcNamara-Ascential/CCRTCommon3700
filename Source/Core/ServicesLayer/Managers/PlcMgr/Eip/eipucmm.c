...
BOOL ucmmParseRegisterSessionResp( SESSION* pSession );
#endif
BOOL ucmmRespondRegisterSession( SESSION* pSession );
void ucmmRespondUnRegisterSession( SESSION* pSession );
void ucmmSendConnectedObjectResponse( REQUEST* pRequest );
BOOL ucmmSendUnconnectedObjectResp( REQUEST* pRequest );
...
EtIPListIdentityInfo* gpnListIdentities;                /* Number outstanding List Identity responses*/
EtIPListIdentityInfo  gListIdentities[MAX_SESSIONS];    /* List Identity responses array */
INT32 gnListIdentityIndex;
   ...
   if ( psEncapHdr->lOpt != 0)
    ...
    switch ( psEncapHdr->iCommand )
    {
        ...
        case ENCAP_CMD_LISTIDENTITY:
            ...
            return ucmmRespondListIdentity( pSession, pSession->lHostIPAddr, NULL);
...
**---------------------------------------------------------------------------
*/
void ucmmIssueRegisterSession( SESSION* pSession )
{        
...
void ucmmIssueListServices( SESSION* pSession )
{        
    ENCAPH*        pHdr = ( ENCAPH * )gmsgBuf;
...
{    
    ...
    {
        if ( pSession != NULL )    /* pSession will be NULL for broadcasts */
...
{        
	...
	UINT16_SET(pData, gIDInfo[0].iVendor );            /* Vendor ID */
	pData += sizeof(UINT16);

	UINT16_SET(pData, gIDInfo[0].iProductType);        /* Product Type */
	pData += sizeof(UINT16);

	UINT16_SET(pData, gIDInfo[0].iProductCode);        /* Product Code */
	pData += sizeof(UINT16);

	*pData++ = gIDInfo[0].bMajorRevision;              /* Revisions */
	*pData++ = gIDInfo[0].bMinorRevision;

	UINT16_SET(pData, gIDInfo[0].iStatus);             /* Device Status */
	pData += sizeof(UINT16); 

	UINT32_SET(pData, gIDInfo[0].lSerialNumber);       /* Serial Number */
	pData += sizeof(UINT32);

	*pData++ = bProductNameSize;                    /* Product name size */            
    
	memcpy( pData, gIDInfo[0].productName, bProductNameSize );    /* Product name */            
...
void ucmmParseListIdentityResp(INT32 lBytesReceived, UINT32 lSrcAddress, UINT32 lHostAddress)
{
	...
	UINT16   i, iNumberOfObjects, iTargetCode, 
			 iObjectLength, iProtocolVersion;
	...
	for (i = 0; i < iNumberOfObjects; i++)
	{
		...
		/* start filling out the the List Identity Response to send
		   to the application */
...
#ifdef CONNECTION_STATS
	gSystemStats.iNumSentClass3Messages++;
#endif
}
#endif
#ifndef EIP_NO_ORIGINATE_UCMM
...
#ifdef CONNECTION_STATS
	gSystemStats.iNumSentUCMMMessages++;
#endif
...
void ucmmSendObjectResponse( REQUEST* pRequest )
{
    if (pRequest->nType == CLASS3_REQUEST)
        ucmmSendConnectedObjectResponse( pRequest );
    else
        ucmmSendUnconnectedObjectResp( pRequest );
}
...
void ucmmSendUnconnectedGroupResponse( REQUEST_GROUP* pRequestGroup )
{
	...
	UINT16         iServices = 0,
				   iOffset = 0,
				   iTagCount = 2,
				   iTagSize, iDefaultLength, iExtError;
    CPF_TAG        tag;
    UINT8*         pData;            
    ENCAP_DT_HDR   dataHdr;
    REPLY_HEADER   replyHdr;
...
BOOL ucmmPassUnconnectedSendRequest( REQUEST* pRequest )
{
	...
	pSession = sessionFindAddressEstablished( pRequest->lIPAddress,
                                              pRequest->lHostIPAddr,
                                              Outgoing );
    ...
    *pData++ = pRequest->bOpenTimeoutTicks;
    ...
    if ( pRequest->cfg.objectCfg.iDataSize )
    {
        ...
        if ( pRequest->cfg.objectCfg.iDataSize % 2 )                                    /* Insert the pad byte if data length is odd */
            *pData++ = 0;
    }
    ...
    *pData++ = 0;
    ...
    if ( pRequest->iExtendedPathSize % 2 )    /* Put the pad byte in if necessary */
        *pData = 0;
