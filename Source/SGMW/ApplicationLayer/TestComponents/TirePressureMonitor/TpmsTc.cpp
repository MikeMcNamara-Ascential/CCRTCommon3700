...
const string TpmsTc<ModuleType>::CommandTestStep(const string &value)
{
	...
	Log(LOG_FN_ENTRY, "TpmsTc::CommandTestStep(value: %s) - Enter", value.c_str());
	try
	{
		if(TpmEquipped())
		{
	...
	Log(LOG_FN_ENTRY, "TpmsTc::CommandTestStep(value: %s) - Exit", value.c_str());
	return result;
}
...
const string TpmsTc<ModuleType>::Register(void)
{
	...
	Log(LOG_FN_ENTRY, "TpmsTc::Register() - Enter");
	...
	Log(LOG_FN_ENTRY, "TpmsTc::Register() - Exit");
	return result;
}
...
{
	string result(BEP_TESTING_STATUS);
	Log(LOG_FN_ENTRY, "TpmsTc::CheckTpmsResults() - Enter");
	if(!ShortCircuitTestStep())
	{   
		BEP_STATUS_TYPE status = BEP_STATUS_ERROR;
		TestResultDetails details;
		// Read the frame counter information
		char buff[32];
		status = m_vehicleModule.ReadModuleData("ReadFrameCounts", m_finalFrameCounts);
		if(BEP_STATUS_SUCCESS == status)
		{
			for(UINT8 index = 0; index < GetRollerCount(); index++)
			{
				details.AddDetail(rollerName[index]+"FinalFrameCount", 
								  CreateMessage(buff, sizeof(buff), "%d", m_finalFrameCounts[index]), "");
				UINT16 frameDiff = m_finalFrameCounts[index] - m_initialFrameCounts[index];
				string tempResult = (frameDiff > 0) ? testPass : testFail;
				details.AddDetail(rollerName[index]+"FrameCountDiff", 
								  CreateMessage(buff, sizeof(buff), "%d", frameDiff), "");
				details.AddDetail(rollerName[index]+"Result", tempResult, "");
				Log(LOG_DEV_DATA, "%s result: %s - initial: %d, final: %d, diff: %d",
					rollerName[index].c_str(), tempResult.c_str(), m_initialFrameCounts[index],
					m_finalFrameCounts[index], frameDiff);
				UpdateResult(tempResult, result);
				SystemWrite(rollerName[index]+"TpmsCount", frameDiff);
				SystemWrite(rollerName[index]+"TpmsCountBgColor", string(tempResult == testPass ? "green" : "red"));
			}
		}
		else
		{
			Log(LOG_ERRORS, "Error reading front frame count information");
			result = testFail;
		}
		// Report the results
	...
	{
		Log(LOG_DEV_DATA, "Not checking TPMS results");
		result = testSkip;
	}
...
{
	string result(BEP_TESTING_STATUS);
	Log(LOG_DEV_DATA, "TpmsTc::StartSpeedTimer() - Enter");
	if(!ShortCircuitTestStep())
	{   // Get the initial frame counts from the TPMS
		m_initialFrameCounts.reserve(GetRollerCount());
		m_finalFrameCounts.reserve(GetRollerCount());
		TestResultDetails details;
		if(BEP_STATUS_SUCCESS == m_vehicleModule.ReadModuleData("ReadFrameCounts", m_initialFrameCounts))
		{
			char buff[32];
			for(UINT8 index = 0; index < GetRollerCount(); index++)
			{
				details.AddDetail(rollerName[index]+"InitialFrameCount", 
								  CreateMessage(buff, sizeof(buff), "%d", m_initialFrameCounts[index]), "");
			}
			result = testPass;
		}
		else
		{
			result = testFail;
			Log(LOG_ERRORS, "Failure reading initial frame counts from the module");
		}
	...
	Log(LOG_DEV_DATA, "TpmsTc::StartSpeedTimer() - Exit");
	return result;
}
...
string TpmsTc<ModuleType>::WaitForTimeAtSpeedComplete(void)
{
	...
	Log(LOG_FN_ENTRY, "TpmsTc::WaitForTimeAtSpeedComplete() - Enter");
	if(!ShortCircuitTestStep())
	{
	...
	Log(LOG_FN_ENTRY, "TpmsTc::WaitForTimeAtSpeedComplete() - Exit");
	return result;
}
...
INT32 TpmsTc<ModuleType>::TpmUpdateSpeedTime(void)
{
	...
	if(currentSpeed >= GetParameterFloat("TpmSpeedThreshold"))
	{
