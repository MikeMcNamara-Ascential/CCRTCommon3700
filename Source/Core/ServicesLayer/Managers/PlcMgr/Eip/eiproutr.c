...
/* local functions */
void routerParseObjectRequest( SESSION* pSession, CPF_PKT_TAG* pPacketTag, UINT8* pData, UINT16 iDataSize, INT32 nGroupIndex, CPF_SOCKADDR_TAG* otTag, CPF_SOCKADDR_TAG* toTag, UINT32 lContext1, UINT32 lContext2  );
void routerParseClassInstanceRqst( REQUEST* pRequest );
void routerParseClassRequest( REQUEST* pRequest );
void routerParseInstanceRequest( REQUEST* pRequest, UINT32 iInstance );
...
void routerProcessMultipleServiceRequest( REQUEST* pRequest);
...
#endif
#ifdef ET_IP_SCANNER
void routerLogClass3Response(CONNECTION* pConnection, CPF_PKT_TAG* pktTag );
#endif

/* Supported by Message Router Class objects */
UINT16    routerObjects[ROUTER_OBJECT_COUNT] = { ID_CLASS, ROUTER_CLASS, ASSEMBLY_CLASS, CONNMGR_CLASS, 
#ifdef EIP_QOS
	QOS_CLASS,
#endif
	PCCC_CLASS, 
	...
	CONFIG_CLASS, 
#endif
	PORT_CLASS, TCPIP_CLASS, ENETLINK_CLASS };

UINT16 routerOptionalAttributes[ROUTER_ATTR_OPTIONAL_COUNT] = {ROUTER_ATTR_CLASS_LIST, ROUTER_ATTR_MAX_CONNECTIONS};
UINT16 routerOptionalServices[ROUTER_SVC_OPTIONAL_COUNT] = {SVC_MULTIPLE_SVC_REQUEST};
	...
	case CPF_TAG_PKT_UCMM:
        ...
        if ( (*pktTag.pPacketPtr) == (SVC_MULTIPLE_SVC_REQUEST | REPLY_BIT_MASK) )        /* This is reply to our multiple service request */
		{
			for( pRequestGroup = gRequestGroups; pRequestGroup < gpnRequestGroups; pRequestGroup++ )       
            {
				...
				{
#ifdef CONNECTION_STATS
					gSystemStats.iNumRecvUCMMMessages++;
#endif
		...
		if ( (*pktTag.pPacketPtr) & REPLY_BIT_MASK )        /* This is some reply */
        {
				...
				(bFoundUCMMRequest == FALSE))
			{
				...
				gSystemStats.iNumRecvUCMMMessages++;
#endif
				return scanmgrProcessFwdOpenReply( pSession, &pktTag, &otTag, &toTag );        
			}

			if ((*pktTag.pPacketPtr == (FWD_CLOSE_CMD_CODE | REPLY_BIT_MASK)) &&    /* This is a Forward Open reply */
				(bFoundUCMMRequest == FALSE))
			{
#ifdef CONNECTION_STATS
				gSystemStats.iNumRecvUCMMMessages++;
		...
		routerParseObjectRequest(pSession, &pktTag, pktTag.pPacketPtr, pktTag.sTag.iTag_length, INVALID_REQUEST_INDEX, &otTag, &toTag, pHdr->lContext1, pHdr->lContext2);
#ifdef CONNECTION_STATS
		gSystemStats.iNumRecvUCMMMessages++;
#endif
        return TRUE;
...
#ifdef CONNECTION_STATS
						gSystemStats.iNumRecvClass3Messages++;
#endif
...
void routerLogResponseGroup( REQUEST_GROUP* pRequestGroup, CPF_PKT_TAG* pktTag )
{
	...
	if (pktTag->sTag.iTag_length - REPLY_HEADER_SIZE - nExtErrorLen < 0)
	{
		DumpStr1(TRACE_LEVEL_ERROR, TRACE_TYPE_EXPLICIT, pRequestGroup->lHostIPAddr, pRequestGroup->lIPAddress,
...
void routerParseObjectRequest( SESSION* pSession, CPF_PKT_TAG* pPacketTag, UINT8* pData, UINT16 iDataSize, INT32 nGroupIndex, CPF_SOCKADDR_TAG* otTag, CPF_SOCKADDR_TAG* toTag, UINT32 lContext1, UINT32 lContext2  )
{
    ...
    pRequest = requestNewIncoming( pPacketTag->bConnected ? CLASS3_REQUEST : OBJECT_REQUEST,
                            FALSE,
                            pSession->nSessionId );    /* Register new incoming object request */
    ...
    pRequest->lIPAddress = pSession->lClientIPAddr;                                                
    ...
    iPDUSize = (UINT16)pduParse( pData, &pRequest->cfg.objectCfg.PDU, TRUE, 0);
...
{        
    ...
    UINT16      iTagSize;
    UINT32      iClass, iInstance, iAttribute, iMember, lPort, iSlot;
					...
					else
					{
						...
						if (iTagSize > 0)
						{
							if (iSlot != giBackplaneSlot)
							{
								...
								pdu_hdr.bSize = (UINT8)(pRequest->iExtendedPathSize/2);
...
void routerSendClassAttrAll( REQUEST* pRequest )
{
	...
	EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, sizeof(UINT16)*(5+ROUTER_ATTR_OPTIONAL_COUNT+ROUTER_SVC_OPTIONAL_COUNT) );
	...
	UINT16_SET(pData, ROUTER_ATTR_MAX_CONNECTIONS);
}
...
void routerSendClassAttrSingle( REQUEST* pRequest)
{
    ...
    switch( iAttribute )
    {
        ...
        case ROUTER_CLASS_ATTR_MAX_INST_ATTR:
            iVal = ENCAP_TO_HS(ROUTER_ATTR_MAX_CONNECTIONS);
            EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, (UINT8*)&iVal, sizeof(UINT16) );        
			break;
        default:
...
void routerSendInstanceAttrAll( REQUEST* pRequest)
{
	...
	EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, (UINT16)((sizeof(UINT16)*(ROUTER_OBJECT_COUNT+glClientAppProcClassNbr)) + sizeof(UINT16) + sizeof(UINT16)));
    ...
    UINT16_SET( pData, (ROUTER_OBJECT_COUNT+glClientAppProcClassNbr));
    pData += sizeof(UINT16);
...
void routerSendInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance )
{
    ...
    switch( iAttribute )
    {
        case ROUTER_ATTR_CLASS_LIST:            
            EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, (UINT16)((sizeof(UINT16)*(ROUTER_OBJECT_COUNT+glClientAppProcClassNbr)) + sizeof(UINT16)));
            if ( pRequest->cfg.objectCfg.pData != NULL )
            {
                pData = pRequest->cfg.objectCfg.pData;
...
void routerProcessMultipleServiceRequest( REQUEST* pRequest)
{
	...
	pRequestGroup = requestGroupNewIncoming( pRequest->nType,
                                        pSession->nSessionId );    /* Register new incoming object request group */
		 ...
		 i++, pOffset += sizeof(UINT16))
	{
		...
		routerParseObjectRequest(pSession, &PacketTag, pRequest->cfg.objectCfg.pData + iRequestDataOffset, iRequestDataSize, 
