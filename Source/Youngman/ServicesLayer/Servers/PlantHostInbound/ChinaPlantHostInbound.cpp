//******************************************************************************
// Description:
//   This defines the ChinaPlantHostInbound object.
//
//===========================================================================
...
//-----------------------------------------------------------------------------
ChinaPlantHostInbound::ChinaPlantHostInbound() : PlantHostInbound(), m_secondarySelectedData(NULL)
{
}

//-----------------------------------------------------------------------------
ChinaPlantHostInbound::~ChinaPlantHostInbound()
{
}

//-----------------------------------------------------------------------------
void ChinaPlantHostInbound::LoadAdditionalConfigurationItems(const XmlNode *document)
...
//-----------------------------------------------------------------------------
const string ChinaPlantHostInbound::LoadVehicleBuildFromFile(const string &vin,
                                                             ...
                                                             const bool &updateStatus)
{
    ...
    std::string secondarySelectionData = GetDataTag("SecondarySelectionDataTag");
    const XmlNodeMap *manualDataMap = &m_manualSecondaryData->getChildren();

    Log(LOG_FN_ENTRY,"ChinaPlantHostInbound::LoadVehicleBuildFromFile() - Enter");
    try
    {
        ...
        // Read the secondary seldction data that was scanned
        ...
        //std::string wheelbaseMm = buildData.getNode("Wheelbase")->getValue();
    ...
    Log(LOG_FN_ENTRY,"ChinaPlantHostInbound::LoadVehicleBuildFromFile() complete, returning: %s\n", status.c_str());

    return(status);
}

const std::string ChinaPlantHostInbound::LoadVehicleBuildRecord(const std::string &vin, 
                                                           XmlNodeMap &buildData, 
                                                           const bool updateStatus)
...
const std::string ChinaPlantHostInbound::Publish(const XmlNode *node)
{
