...
//
// LOG:
//    $Log: /Tata/Source/Tata/ApplicationLayer/TestComponents/SoundLevel/TataSoundLevelTCMain.cpp $
// 
// 2     6/27/11  2:17p Rsmith
// Took ChanganSoundLevelTCMain from TataSoundLevelTCMain
//
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		component = new ChanganSoundLevelTC;
	...
	{
		printf("ChanganSoundLevelTC -%s: XmlExcpetion: %s", clp.GetConfigFile().c_str(), XmlErr.what());
	}
	catch(BepException &BepErr)
	{
		printf("ChanganSoundLevelTC -%s: BepExcpetion: %s", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("ChanganSoundLevelTC -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}

	if( NULL != component)	delete component;

	if(clp.IsDebugOn())
		printf("ChanganSoundLevelTC (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
