...
BEP_STATUS_TYPE Bosch8Module<ProtocolFilterType>::GetInfo(string methodName, bool &value) throw(ModuleException)
{
    ...
    else if(methodName == "CheckModuleType") status = ReadModuleData(methodName,value);
...
BEP_STATUS_TYPE Bosch8Module<ProtocolFilterType>::EnterDiagnosticMode() throw(ModuleException)
{
    ...
    Log( LOG_FN_ENTRY, "Enter Bosch8Bodule::EnterDiagnosticMode()\n");
    ...
    Log( LOG_FN_ENTRY, "Exit Bosch8Bodule::EnterDiagnosticMode(), status=%d\n", status);
