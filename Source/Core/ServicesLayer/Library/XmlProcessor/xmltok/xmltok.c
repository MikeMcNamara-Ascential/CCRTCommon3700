...
#define UTF8_GET_NAMING2(pages, byte) (namingBitmap[((pages)[(((byte)[0]) >> 2) & 7] << 3) + ((((byte)[0]) & 3) << 1) + ((((byte)[1]) >> 5) & 1)] & (1 << (((byte)[1]) & 0x1F)))

/* A 3 byte UTF-8 representation splits the characters 16 bits
between the bottom 4, 6 and 6 bits of the bytes.
We need 8 bits to index into pages, 3 bits to add to that index and
5 bits to generate the mask. */
#define UTF8_GET_NAMING3(pages, byte) (namingBitmap[((pages)[((((byte)[0]) & 0xF) << 4) + ((((byte)[1]) >> 2) & 0xF)] << 3) + ((((byte)[1]) & 3) << 1) + ((((byte)[2]) >> 5) & 1)] & (1 << (((byte)[2]) & 0x1F)))
...
#define UTF8_INVALID3(p) ((*p) == 0xED ? (((p)[1] & 0x20) != 0) : ((*p) == 0xEF ? ((p)[1] == 0xBF && ((p)[2] == 0xBF || (p)[2] == 0xBE)) : 0))
...
#define BYTE_TYPE(enc, p) (((const struct normal_encoding *)(enc))->byteType(enc, p))
#else
#define BYTE_TYPE(enc, p) SB_BYTE_TYPE(enc, p)
#endif

#ifdef XML_MIN_SIZE
#define BYTE_TO_ASCII(enc, p) (((const struct normal_encoding *)(enc))->byteToAscii(enc, p))
static
...
#ifdef XML_MIN_SIZE
#define IS_NAME_CHAR_MINBPC(enc, p) (((const struct normal_encoding *)(enc))->isNameMin(enc, p))
#define IS_NMSTRT_CHAR_MINBPC(enc, p) (((const struct normal_encoding *)(enc))->isNmstrtMin(enc, p))
#else
...
#ifdef XML_MIN_SIZE
#define CHAR_MATCHES(enc, p, c)  (((const struct normal_encoding *)(enc))->charMatches(enc, p, c))
static
...
#define DEFINE_UTF16_TO_UTF16(E)  static  void E ## toUtf16(const ENCODING *enc,  const char **fromP, const char *fromLim,  unsigned short **toP, const unsigned short *toLim)  {  /* Avoid copying first half only of surrogate */  if (fromLim - *fromP > ((toLim - *toP) << 1)  && (GET_HI(fromLim - 2) & 0xF8) == 0xD8) fromLim -= 2; for (; *fromP != fromLim && *toP != toLim; *fromP += 2) *(*toP)++ = (GET_HI(*fromP) << 8) | GET_LO(*fromP); }
