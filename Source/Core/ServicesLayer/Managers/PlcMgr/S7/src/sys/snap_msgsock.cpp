...
//    if (send(FSocket, (char*)Data, Size, MSG_NOSIGNAL)==Size)
	if (send(FSocket, (char*)Data, Size, 0)==Size)
        return 0;
    else
        Result =SOCKET_ERROR;
...
//        SizeRecvd=recv(FSocket ,(char*)Data ,BufSize ,MSG_NOSIGNAL );
		SizeRecvd=recv(FSocket ,(char*)Data ,BufSize ,0 );
...
//        BytesRead=recv(FSocket, (char*)Data, Size, MSG_NOSIGNAL);
		BytesRead=recv(FSocket, (char*)Data, Size, 0);
        if (BytesRead==0)
            LastTcpError = WSAECONNRESET;  // Connection reset by Peer
        else
            if (BytesRead<0)
...
//        BytesRead=recv(FSocket, (char*)Data, Size, MSG_PEEK | MSG_NOSIGNAL );
		BytesRead=recv(FSocket, (char*)Data, Size, MSG_PEEK );
        if (BytesRead==0)
            LastTcpError = WSAECONNRESET;  // Connection reset by Peer
        else
            if (BytesRead<0)
...
//    if (send(FSocket, (char*)SendPacket, sizeof(TIcmpPacket), MSG_NOSIGNAL)!=int(sizeof(TIcmpPacket)))
	if (send(FSocket, (char*)SendPacket, sizeof(TIcmpPacket), 0)!=int(sizeof(TIcmpPacket)))
        return false;
    // Wait for a reply
    if (!CanRead(Timeout))
...
//    if (recv(FSocket, (char*)&IcmpBuffer, ICmpBufferSize, MSG_NOSIGNAL)<int(sizeof(TIcmpReply)))
	if (recv(FSocket, (char*)&IcmpBuffer, ICmpBufferSize, 0)<int(sizeof(TIcmpReply)))
        return false;
    // Check the answer
