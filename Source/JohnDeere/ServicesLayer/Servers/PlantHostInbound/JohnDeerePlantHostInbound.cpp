...
#include "JohnDeerePlantHostInbound.h"

//-----------------------------------------------------------------------------
JohnDeerePlantHostInbound::JohnDeerePlantHostInbound() : PlantHostInbound(), 
	m_secondarySelectedData(NULL)
{
}

//-----------------------------------------------------------------------------
JohnDeerePlantHostInbound::~JohnDeerePlantHostInbound()
{
}

//-----------------------------------------------------------------------------
void JohnDeerePlantHostInbound::LoadAdditionalConfigurationItems(const XmlNode *document)
...
//-----------------------------------------------------------------------------
const string JohnDeerePlantHostInbound::LoadVehicleBuildFromFile(const string &vin,
																...
																const bool &updateStatus)
{
	...
	Log(LOG_FN_ENTRY,"JohnDeerePlantHostInbound::LoadVehicleBuildFromFile() - Enter");
	try
	{
	...
	Log(LOG_FN_ENTRY,"JohnDeerePlantHostInbound::LoadVehicleBuildFromFile() complete, returning: %s\n", status.c_str());

	return(status);
}

//-----------------------------------------------------------------------------
const string JohnDeerePlantHostInbound::LoadVehicleBuildRecord(const string &vin, 
															  XmlNodeMap &buildData, 
															  const bool updateStatus)
{	// Clear the vehicle build record before reloading it
	Log(LOG_FN_ENTRY,"Enter JohnDeerePlantHostInbound::LoadVehicleBuildRecord(%s)\n",vin.c_str());
	...
	{	// Add information derived from the vehicle build information to the vehicle build record
		...
		Log(LOG_DEV_DATA, "JohnDeerePlantHostInbound::LoadVehicleBuildRecord - Loaded vehicleBuild with:\n");
	...
	Log(LOG_FN_ENTRY,"JohnDeerePlantHostInbound::LoadVehicleBuildRecord() returning %s\n", status.c_str());
	return(status);
}

//-----------------------------------------------------------------------------
const string JohnDeerePlantHostInbound::Publish(const XmlNode *node)
{
	string result = BEP_ERROR_RESPONSE, response, loadResult;
	Log("JohnDeerePlantHostInbound::Publish(%s)\n", node->ToString().c_str());
...
//-----------------------------------------------------------------------------
void JohnDeerePlantHostInbound::UpdateInputServerState(void)
