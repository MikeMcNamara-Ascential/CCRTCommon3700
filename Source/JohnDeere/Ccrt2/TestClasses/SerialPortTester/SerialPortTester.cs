...
namespace SerialPortTester
{
    public partial class SerialPortTester : Form
    {
        ...
        private PortSettings GetPortSettings()
        {
            ...
            portSettings.DeviceType = m_devTypeComboBox.SelectedItem.ToString();
            return portSettings;
        }
        ...
        /// <summary>
        /// Send the TX HEX data to the serial port.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void m_txHexButton_Click(object sender, EventArgs e)
        {
            char[] txBuf = new char[128];
            int txLen = 0;
            string [] bytesToSend = m_txHexTextBox.Text.Split(new char[] {' ',',',':'}, 128, StringSplitOptions.RemoveEmptyEntries);
            foreach(string s in bytesToSend)
            {
                txBuf[txLen] = (char)Convert.ToByte(s,16);
                txLen++;
            }
            m_serialPort.WriteSerialData(txBuf, txLen);
        }
        ...
        private void m_readPortButton_Click(object sender, EventArgs e)
        {
            ...
            {
                rxHexString += "0x" + Convert.ToString((byte)rxData[i],16) + " ";
            }
            if (byteCount > 0)
            {
                m_rxdataTextBox.Text = new String(rxData);
                m_rxHexTextBox.Text = new String(rxHexString.ToCharArray());
            }
            else
            {
                m_rxdataTextBox.Text = "No data received";
                m_rxHexTextBox.Text = "No data received";
            }
        }
    }
}
