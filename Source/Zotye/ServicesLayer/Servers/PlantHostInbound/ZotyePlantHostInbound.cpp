//******************************************************************************
// Description:
//   This defines the ZotyePlantHostInbound object.
//
//===========================================================================
// Copyright (c) 20015- Burke E. Porter Machinery
// All Rights Reserved
//
...
#include "ZotyePlantHostInbound.h"

//-----------------------------------------------------------------------------
ZotyePlantHostInbound::ZotyePlantHostInbound() : PlantHostInbound(), 
	m_secondarySelectedData(NULL)
{
}

//-----------------------------------------------------------------------------
ZotyePlantHostInbound::~ZotyePlantHostInbound()
{
}

//-----------------------------------------------------------------------------
void ZotyePlantHostInbound::LoadAdditionalConfigurationItems(const XmlNode *document)
...
//-----------------------------------------------------------------------------
const string ZotyePlantHostInbound::LoadVehicleBuildFromFile(const string &vin,
																...
																const bool &updateStatus)
{
	...
	Log(LOG_FN_ENTRY,"ZotyePlantHostInbound::LoadVehicleBuildFromFile() - Enter");
	try
	{
	...
	Log(LOG_FN_ENTRY,"ZotyePlantHostInbound::LoadVehicleBuildFromFile() complete, returning: %s\n", status.c_str());

	return(status);
}

//-----------------------------------------------------------------------------
const string ZotyePlantHostInbound::LoadVehicleBuildRecord(const string &vin, 
															  XmlNodeMap &buildData, 
															  const bool updateStatus)
...
//-----------------------------------------------------------------------------
const string ZotyePlantHostInbound::Publish(const XmlNode *node)
{
	string result = BEP_ERROR_RESPONSE, response, loadResult;
	Log("ZotyePlantHostInbound::Publish(%s)\n", node->ToString().c_str());
...
//-----------------------------------------------------------------------------
void ZotyePlantHostInbound::UpdateInputServerState(void)
