...
/* local functions */
void connectionInitialize( CONNECTION* pConnection );             /* Initialize connection object */
#ifndef EIP_LARGE_CONFIG_OPT
BOOL connectionIsTimedOut( CONNECTION* pConnection );             /* Did we time out */
#endif
BOOL connectionAssignSocketConnGroup( CONNECTION* pConnection );  /* Assign a "socket group" for class1 connections */

#ifdef EIP_LARGE_CONFIG_OPT
void ProcessConnectionPoll(CONNECTION *pConnection);
void OnConnectionProductionTimer(CONNECTION *pConnection);
void OnConnectionInhibitTimer(CONNECTION *pConnection);
void OnConnectionTimeoutTimer(CONNECTION *pConnection);
void OnConnectionConfigTimer(CONNECTION *pConnection);
void OnConnectionDelayTimer(CONNECTION *pConnection);
#ifdef CONNECTION_STATS
void OnConnectionStatsDelayTimer(CONNECTION *pConnection);
#endif
void ConnectionProduce(CONNECTION *pConnection);
#endif

CONNECTION  gConnections[MAX_CONNECTIONS];
CONNECTION* gpnConnections;
#ifdef CONNECTION_STATS
EtIPSystemStats gSystemStats;
#endif
...
#ifdef CONNECTION_STATS
	memset(&gSystemStats, 0, sizeof(EtIPSystemStats));
#endif
...
void connectionInitialize( CONNECTION* pConnection )
{
	...
	*pDataSent = FALSE;
    ...
    {    
        ...
        case ConnectionConfiguring:
        {
            switch( pConnection->lConfigurationState )
            {
                ...
                case ConfigurationWaitingForSession:
                {
                    ...
                    if ( pSession == NULL || IS_TICK_GREATER( gdwTickCount, pConnection->lConfigurationTimeoutTick ) )
                    {    
                    ...
                    else if (pSession->lState == OpenSessionEstablished )
                    {
                        if ( scanmgrOutgoingConnection(pConnection ) )
                        {
                            pConnection->lConfigurationState = ConfigurationWaitingForForwardOpenResponse;                        
        ...
        {                
            ...
            if ( connectionIsTimedOut( pConnection ) )        /* Is connection timed out */
            {
        ...
        case ConnectionClosing:
            switch( pConnection->lConfigurationState )
            {
				...
				case ConfigurationClosingTimeOut: 
                    if ( IS_TICK_GREATER(gdwTickCount, pConnection->lConfigurationTimeoutTick ) )
                    {
						connectionRemove( pConnection, FALSE );    /* Forward Close response did not come - close now */
						return TRUE;
					}
                    break;
...
BOOL connectionAssignClass1Socket( CONNECTION* pConnection )
{
    ...
    {
        *pSocket = socketClass1Init( FALSE,

                                  COMMON_BUFFER_SIZE,
                                  pTcpipInterface->InterfaceConfig.lIpAddr,
...
{    
    ...
    if ( pLoopConnection == gpnConnections )    /* This was the last open connection for this Host Address/Conn Group combination - close the socket */
    {        
...
**---------------------------------------------------------------------------
*/
BOOL connectionAssignSocketConnGroup( CONNECTION* pConnection )
{
	UINT32 i;
	CONNECTION* pLoopConnection;
	INT32 iGroupCount = 0;
...
void connectionGoOffline( CONNECTION* pConnection, BOOL bTimeOut )
{
	...
	if ((connectionIsIOConnection(pConnection)) && 
		(!pConnection->cfg.bOriginator) &&
		(pConnection->cfg.iTargetProducingConnPoint != HEARTBEAT_CONN_POINT))
	{
			...
			if (pConnection->cfg.iProducingConnectionType == Multicast)
			{
				for ( pLoopConnection = gConnections; pLoopConnection < gpnConnections; pLoopConnection++ )
				{
					...
					if ( pLoopConnection != pConnection && 
						 ...
						 (pLoopConnection->cfg.iTargetConsumingConnPoint == LISTEN_ONLY_CONN_POINT || bTimeOut ) &&
		...
		else if ( pConnection->cfg.iTargetConsumingConnPoint == HEARTBEAT_CONN_POINT )
		{	
...
void connectionRemove( CONNECTION* pConnection, BOOL bTimeOut )
{
	...
	if ( nInstance != (INT32)INVALID_CONN_INSTANCE && nInstance != 0 && pConnection->lConnectionState != ConnectionNonExistent )
		notifyEvent( NM_CONNECTION_CLOSED, nInstance, 0 );
...
**---------------------------------------------------------------------------
*/
BOOL connectionIsTimedOut( CONNECTION* pConnection )
{
...
INT32 connectionGetConnectedCount(void)
{
    CONNECTION* pConnection;
    INT32  nCount = 0;
...
void connectNotifyNewRedundantMaster(CONNECTION* pConnection)
{
	CONNECTION *pLoopConnection;
...
void connectNotifyNoRedundantMaster(CONNECTION* pConnection)
{
	CONNECTION *pLoopConnection;
...
BOOL connectionConvertEPATHToInternal(UINT8* pByteArrayEPATH, INT32 iEPATHSize, EtIPEPATHConnectionConfig* pcfg, CONNECTION_CFG* pConfig, INT32* piError)
{
	...
	{
		pConfig->targetConsumingConnTag = (UINT8*)eTag;
		...
		if (pConfig->iTargetProducingConnTagSize != 0)
			pConfig->targetProducingConnTag = (UINT8*)eTag;
	}
	else
	{
...
{    
    ...
    if ( iTagSize )
	{
		...
		if ((pConfig->iTargetConsumingConnPoint != HEARTBEAT_CONN_POINT) && 
			(pConfig->iTargetConsumingConnPoint != LISTEN_ONLY_CONN_POINT)
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
    ...
    if ( iTagSize )
	{
		...
		if ((pConfig->iTargetProducingConnPoint != HEARTBEAT_CONN_POINT)
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
	...
	if ( pConfig->iTargetConsumingConnPoint == INVALID_CONN_POINT || pConfig->iTargetProducingConnPoint == INVALID_CONN_POINT )
	{
		...
		if ( pConfig->iTargetConsumingConnPoint == INVALID_CONN_POINT && pConfig->iTargetProducingConnPoint == INVALID_CONN_POINT )
		{
			...
			{
				if ( iAssembly2TagSize == 0 )							/* If the second tag is specified - it must be valid. Otherwise reject. */
				{
					if ( pAssemblyTag1->iType & AssemblyProducing )		/* In case of both producing and consuming assembly and only one tag and no connection point - always default to producing */
					{
						pConfig->iTargetConsumingConnPoint = HEARTBEAT_CONN_POINT;				
					...
					else if ( pAssemblyTag1->iType & AssemblyConsuming )
					{
						...
						pConfig->iTargetConsumingConnTagSize = iAssembly1TagSize;
						pConfig->targetConsumingConnTag = EtIP_malloc( (UINT8*)assembly1Tag, iAssembly1TagSize );
						pConfig->iTargetProducingConnPoint = HEARTBEAT_CONN_POINT;
					}
				}
			}
		}
		...
		{			
			if ( pAssemblyTag1 != NULL )									 /* A tag specified */
			{
				if ( pConfig->iTargetConsumingConnPoint == HEARTBEAT_CONN_POINT )
				{
				...
				else if ( pConfig->iTargetConsumingConnPoint == LISTEN_ONLY_CONN_POINT )
				{
					if ( pAssemblyTag1->iType & AssemblyProducing )
					{
						pConfig->iTargetProducingConnPoint = pAssemblyTag1->iInstance;
...
EtIPNetworkStatus connectionGetCurrentNetLEDState(UINT32 lIPAddress)
{
	...
	for ( pLoopConnection = gConnections; pLoopConnection < gpnConnections; pLoopConnection++ )
	{
		...
		if ((pLoopConnection->cfg.bOriginator == FALSE) &&
			...
			(pLoopConnection->cfg.iTargetConsumingConnPoint != HEARTBEAT_CONN_POINT) &&
            (pLoopConnection->cfg.iTargetConsumingConnPoint != LISTEN_ONLY_CONN_POINT))
		{
...
#define MAX_TIMER_PER_TASK  15
	...
	do
	{
		...
		{
			OnConnectionProductionTimer(pConnection);
			prodCount++;
		}
...
void ProcessConnectionPoll(CONNECTION *pConnection)
{
		 ...
		 pConnection->lConnectionState != ConnectionClosing )
	{
		...
		{
			connectionDelayPending( pConnection );      /* trigger any other connections on same IP that are waiting on us */
			connectionRemove( pConnection, FALSE );
			return;
		}
	}
...
void OnConnectionDelayTimer(CONNECTION *pConnection)
{
	...
	case ConnectionClosing:
		switch( pConnection->lConfigurationState )  
		{
			case ConfigurationClosing:  /* delay before close */            
				connListQueueCnxnPollEntry(pConnection);
				break;
			default :
				DumpStr0(TRACE_LEVEL_ERROR, TRACE_TYPE_CONNECTION, pConnection->cfg.lHostIPAddr, pConnection->cfg.lIPAddress, 
...
void OnConnectionStatsDelayTimer(CONNECTION *pConnection)
{
	switch( pConnection->lConnectionState )
	{    
	case ConnectionEstablished:                  /* turn on stats collection after the initial delay */    
		pConnection->bCollectingStats = TRUE;
		break;
