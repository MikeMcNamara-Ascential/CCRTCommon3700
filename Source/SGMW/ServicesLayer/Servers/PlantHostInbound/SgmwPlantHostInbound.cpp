...
//-----------------------------------------------------------------------------
SgmwPlantHostInbound::SgmwPlantHostInbound() : PlantHostInbound(), 
	m_secondarySelectedData(NULL)
{
}

//-----------------------------------------------------------------------------
SgmwPlantHostInbound::~SgmwPlantHostInbound()
{
}

//-----------------------------------------------------------------------------
void SgmwPlantHostInbound::LoadAdditionalConfigurationItems(const XmlNode *document)
...
//-----------------------------------------------------------------------------
const string SgmwPlantHostInbound::LoadVehicleBuildFromFile(const string &vin,
																...
																const bool &updateStatus)
{
	...
	Log(LOG_FN_ENTRY,"SgmwPlantHostInbound::LoadVehicleBuildFromFile() - Enter");
	try
	{
	...
	Log(LOG_FN_ENTRY,"SgmwPlantHostInbound::LoadVehicleBuildFromFile() complete, returning: %s\n", status.c_str());

	return(status);
}

//-----------------------------------------------------------------------------
const string SgmwPlantHostInbound::LoadVehicleBuildRecord(const string &vin, 
															  XmlNodeMap &buildData, 
															  const bool updateStatus)
...
//-----------------------------------------------------------------------------
const string SgmwPlantHostInbound::Publish(const XmlNode *node)
{
	string result = BEP_ERROR_RESPONSE, response, loadResult;
	Log("SgmwPlantHostInbound::Publish(%s)\n", node->ToString().c_str());
...
//-----------------------------------------------------------------------------
void SgmwPlantHostInbound::UpdateInputServerState(void)
