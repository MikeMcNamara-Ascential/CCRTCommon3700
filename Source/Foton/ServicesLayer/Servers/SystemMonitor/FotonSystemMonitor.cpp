//******************************************************************************
//
// $Archive: /CMake Core/Source/Ford/ServicesLayer/Servers/SystemMonitor/FotonSystemMonitor.cpp $
// $Author: Mmcnamar $
//   $Date: 7/12/07 12:34p $
//
// Description:
...
//
// LOG:
//     $Log: /CMake Core/Source/Ford/ServicesLayer/Servers/SystemMonitor/FotonSystemMonitor.cpp $
...
%C [-c/-e][-d] [-ffile] [-py] [-vx] [-r] &
	...
	- Registers name as 'rolls/FotonSystemMonitor' by default
 ...
 */
FotonSystemMonitor::FotonSystemMonitor() : SystemMonitor(), 
 ...
 */
FotonSystemMonitor::FotonSystemMonitor( int argc, char *argv[]) : SystemMonitor( argc, argv),
	m_ICMSpeedLocation( "/dev/Icm/Block/Speed"), m_rollerCount( 4), m_currentDriveAxle(""),
 ...
 * Class destructor
 */
FotonSystemMonitor::~FotonSystemMonitor()
{
}
 ...
 * @since Version 1.0
 */
void FotonSystemMonitor::Initialize(const XmlNode *document)
{
	Log( LOG_FN_ENTRY, "Enter FotonSystemMonitor::Initialize()\n");
	...
	Log( LOG_FN_ENTRY, "Exit FotonSystemMonitor::Initialize()\n");
}
 ...
 * @since Version 2.0
 */
const std::string FotonSystemMonitor::Write(const std::string &tag, const std::string &value)
{
 ...
 * @since Version 2.0
 */
const std::string FotonSystemMonitor::Write(const XmlNode *node)
{
    std::string status;
 ...
 * @since Version 2.0
 */
const std::string FotonSystemMonitor::Command(const std::string &tag, const std::string &value)
{
 ...
 * @since Version 2.0
 */
const std::string FotonSystemMonitor::Command(const XmlNode *node)
{
 ...
 */
void FotonSystemMonitor::UpdateControl(ControlData *ctrl)
{
...
//-----------------------------------------------------------------------------
const std::string FotonSystemMonitor::Publish(const XmlNode *node)
{
 ...
 */
void FotonSystemMonitor::GetCurrentControlData( ControlData *ctrl)
{
	Base3700ControlData *Base3700Ctrl = (Base3700ControlData *)ctrl;
	Log( LOG_FN_ENTRY, "Enter Foton GetCurrentControlData()\n");
...
//-----------------------------------------------------------------------------
const string FotonSystemMonitor::Register(void)
 ...
 */
void FotonSystemMonitor::UpdateDriverMonitor(ControlData *ctrl)
{
 ...
 * @param ctrl Current value of control data needed for system control events
 */
void FotonSystemMonitor::CheckOkToRoll( ControlData *ctrl)
{
	Base3700ControlData *Base3700Ctrl = (Base3700ControlData *)ctrl;
	...
	else
	{
        if (!(ctrl->vehiclePresent))
        {
            Log(LOG_DEV_DATA, "CheckOkToRoll(): No Vehicle Displaying Machine Ready prompt\n");
            DisplayPrompt( 1, "MachineReady", false);
        }/*
 ...
 * @param ctrl Current value of control data needed for system control events
 */
void FotonSystemMonitor::CheckAbort( ControlData *ctrl)
{
//	bool	invalidLicense = (bool)(ReadSubscribeData(INVALID_LICENSE_FATAL_FAULT_TAG) == "1");
	//bool    retainersUp = true;

	Log( LOG_FN_ENTRY, "Enter FotonSystemMonitor::CheckAbort()\n");
	...
	Log( LOG_FN_ENTRY, "Exit FotonSystemMonitor::CheckAbort()\n");
}
#endif

/*
void FotonSystemMonitor::StackLightCheck( ControlData *ctrl)
...
void FotonSystemMonitor::UpdateCommandPhase( ControlData *ctrl)
{
 ...
 * @param ctrl Current value of control data needed for system control events
 */
void FotonSystemMonitor::CheckTesting( ControlData *ctrl)
{
	Base3700ControlData *Base3700Ctrl = (Base3700ControlData *)ctrl;

	Log( LOG_FN_ENTRY, "Enter FotonSystemMonitor::CheckTesting()\n");
    ...
    // If a vehicle just pulled up, prompt to lower elevators
    if(ctrl->vehiclePresent && !m_oldCtrl->vehiclePresent)
    {   
        RemovePrompt(1, "MachineReady");
        DisplayPrompt(1, "LowerElevators");
    }
    ...
    {   // Test just completed, prompt to raise elevators
        Log(LOG_DEV_DATA, "Test just completed, checking rolls down");
        if(!ctrl->rollsDown)
        {   // release vehicle
    ...
    {   // Rolls just lowered, clear the start test flag
        ...
        // If this is a loss comp sequence, clear test in progress data tag
	...
	Log( LOG_FN_ENTRY, "Exit FotonSystemMonitor::CheckTesting()\n");
}

INT32 FotonSystemMonitor::GetWheelSpeeds(float wheelSpeed[])
{
 ...
 * @param ctrl   Current value of control data needed for system control events
 */
void FotonSystemMonitor::UpdateDriveAxle( ControlData *ctrl)
{
	Log(LOG_FN_ENTRY, "Enter FotonSystemMonitor::UpdateDriveAxle()\n");
	...
	Log(LOG_FN_ENTRY, "Exit FotonSystemMonitor::UpdateDriveAxle()\n");
}


INT32 FotonSystemMonitor::GetRollerCount( void)
{
	return(m_rollerCount);
}

void FotonSystemMonitor::SetRollerCount( INT32 rollerCount)
{
	m_rollerCount = rollerCount;
}


ControlData* FotonSystemMonitor::ControlAllocate() const
{
...
void FotonSystemMonitor::ControlFree(ControlData* &controlData) const
{
