...
{  // Set up some variables
    ...
    {
        Log( LOG_ERRORS, "Failed to enter zerospeed in Bosch8AbsTc::EnterDiagnosticMode()\n");
...
string Bosch8TC<ModuleType>::SensorQualityTest(void)
{
    ...
    if(CheckZeroSpeed())
    {
        Log( LOG_DEV_DATA, "Calling Engage() Bosch8AbsTc::SensorQualityTest()\n");
        Engage();
    }
    else
    {
        Log( LOG_DEV_DATA, "Failed to stop in Bosch8AbsTc::SensorQualityTest()\n");        
    }
...
{   // Log the entry and determine if this test needs to be performed
    ...
    Log(LOG_FN_ENTRY, "Bosch8TC::SensorQualityTest() - Enter");
...
string Bosch8TC<ModuleType>::CheckUplineProcessByte(void)
{
    ...
    {
        Log(LOG_ERRORS, "Module Exception in Bosch8TC::CheckUplineProcessByte() - %s\n", 
            moduleException.message().c_str());
        testResult = testSoftwareFail;
        testResultCode = GetFaultCode("SoftwareFailure");
        testDescription = GetFaultDescription("SoftwareFailure");
...
template <class ModuleType>
string Bosch8TC<ModuleType>::CheckEolByte(void)
{
    string testResult = BEP_TESTING_STATUS;
    string testResultCode("0000");
    string testDescription = GetTestStepInfo("Description");
    ...
    Log(LOG_FN_ENTRY, "Enter Bosch8TC::CheckEolByte()\n");
    try
    {
        // 
    ...
    {
        Log(LOG_ERRORS, "Module Exception in Bosch8TC::CheckEolByte() - %s\n", 
            moduleException.message().c_str());
        testResult = testSoftwareFail;
        testResultCode = GetFaultCode("SoftwareFailure");
        testDescription = GetFaultDescription("SoftwareFailure");
    ...
    SendTestResult(testResult, testDescription, testResultCode);
    // Return the test result
    Log(LOG_FN_ENTRY, "Exit Bosch8TC::CheckEolByte()\n");
    return(testResult);
}
...
//-----------------------------------------------------------------------------
template <class ModuleType>
string Bosch8TC<ModuleType>::StartAbsTest(void)
{
    string testResult(BEP_TESTING_RESPONSE);
    Log(LOG_FN_ENTRY, "Bosch8TC::StartAbsTest() - Enter");
...
//-----------------------------------------------------------------------------
template <class ModuleType>
BEP_STATUS_TYPE Bosch8TC<ModuleType>::AnalyzeValveCross(void)
...
//-----------------------------------------------------------------------------
template <class ModuleType>
string Bosch8TC<ModuleType>::WarningLight(void)
{   
    Log(LOG_FN_ENTRY, "Bosch8TC::WarningLight() - Enter");
    string testResult = testPass;
    if(!ShortCircuitTestStep())
    {
        string testResultCode = "0000";
    ...
    // Log the exit and return the result
    Log(LOG_FN_ENTRY, "Bosch8TC::WarningLight() - Exit");
    return testResult;
}
...
template <class ModuleType>
string Bosch8TC<ModuleType>::CheckModuleType(void)
{
    string testResult = BEP_TESTING_STATUS;
    string testResultCode("0000");
    string testDescription = GetTestStepInfo("Description");
    ...
    Log(LOG_FN_ENTRY, "Enter Bosch8TC::CheckModuleType()\n");
    try
    {
        // 
    ...
    {
        Log(LOG_ERRORS, "Module Exception in Bosch8TC::CheckModuleType() - %s\n", 
            moduleException.message().c_str());
        testResult = testSoftwareFail;
        testResultCode = GetFaultCode("SoftwareFailure");
        testDescription = GetFaultDescription("SoftwareFailure");
    ...
    SendTestResult(testResult, testDescription, testResultCode);
    // Return the test result
    Log(LOG_FN_ENTRY, "Exit Bosch8TC::CheckModuleType()\n");
    return(testResult);
}
