...
int main(int argc, char *argv[])
{
	...
	try
	{
		if(clp.IsDebugOn())	printf("Creating The MazdaMachineDataBroker\n");
		server = new MazdaMachineDataBroker();		// create a MazdaMachineDataBroker server
		// create, initialize, and process messages for the named data broker
		if(clp.IsDebugOn())	printf("Initializing The MazdaMachineDataBroker\n");
		server->Initialize(clp.GetConfigFile());
		if(clp.IsDebugOn())  printf("MazdaMachineDataBroker (%d, %s) Up - Listening\nRunning The MazdaMachineDataBroker\n",
                                    BposGetMyTaskId(), clp.GetConfigFile().c_str());
		server->Run();							// process until terminated
	}
	catch(BepException &BepErr)
	{
		printf("MazdaMachineDataBroker -%s: BepException: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("MazdaMachineDataBroker -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}
	if(clp.IsDebugOn())	printf("Deleting The MazdaMachineDataBroker\n");
	if( NULL != server)	delete server;
	if(clp.IsDebugOn())  printf("MazdaMachineDataBroker (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
}
