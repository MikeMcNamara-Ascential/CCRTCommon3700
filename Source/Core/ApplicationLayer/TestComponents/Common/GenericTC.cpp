											  ...
											  string promptName) /* = "AccelerateToTargetSpeed" */
{
	...
	else
	{
		...
		//ensure proper speed range before continuing
		status = WaitForTargetSpeed(targetSpeed, UP, scanDelay);
...
//=============================================================================
BEP_STATUS_TYPE GenericTC::WaitForTargetSpeed(const float targetSpeed, Direction direction, UINT32 wait) /*= 100 */
{
	...
	float currentSpeed;					// the current speed of the vehicle
	// Log the function entry
	...
	{	// Loop to check for target speed
		...
		{	// Get the current roller speeds
			...
			if(((direction == UP) && (currentSpeed >= targetSpeed)) ||
