 ...
 */
PlcEip::PlcEip() : IPlc(),
m_fromPlc( NULL),  m_toPlc( NULL), m_eipPortName( ""),
 ...
 */
PlcEip::PlcEip( const PlcEip &copy) : IPlc( copy),
m_fromPlc( NULL),  m_toPlc( NULL), m_eipPortName( ""),
m_commScanTime( 20)
{
	// Initialize the read/write lock
...
PlcEip::~PlcEip()
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::~PlcEip\n");
	Close();

	// Destroy the read/write lock
	pthread_rwlock_destroy( &m_plcLock);
	Log( LOG_FN_ENTRY, "Exit PlcEip::~PlcEip\n");
}

/**
 * Opens a EIP serial PLC board
 *
 ...
 */
bool PlcEip::Open(uint32_t inputCount, uint32_t outputCount, bool demoMode)
{
	bool    retVal = true;

	Log( LOG_FN_ENTRY, "Enter PlcEip::Open()\n");
	...
	try
	{
		// Launch the EIP comm thread
		LaunchCommThread( m_eipPortName);
	}
	catch( BepException &err)
	{
		Log( LOG_ERRORS, "\tError launching EIP comm thread: %s\n",
			 err.GetReason());
		retVal = false;
	}
	Log( LOG_FN_ENTRY, "Exit PlcEip::Open()\n");
 ...
 */
void PlcEip::Initialize( const XmlNode *plcSetup)
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::Initialize( %s)\n", plcSetup->getName().c_str());

	// Load the additional configuration items needed by the PlcEip object
	LoadAdditionalConfigurationItems(plcSetup);

	Log( LOG_FN_ENTRY, "Exit PlcEip::Initialize()\n");
}

void PlcEip::LoadAdditionalConfigurationItems(const XmlNode *plcSetup)
{
	...
	try
	{
		...
		m_eipPortName = XmlToAscii( tmp);
	}
	catch( XmlException &err)
	{
		m_eipPortName = "PlcEip";
	}
	Log( LOG_DEV_DATA, "EIP PLC Logical Port Name: %s\n", m_eipPortName.c_str());
	...
	Log( LOG_DEV_DATA, "EIP PLC Scan Time: %d ms\n", m_commScanTime);
#if 0
	// Read the EIP source node IDs
	try
	{
		const XmlString& tmp = plcSetup->getChild( XML_T("DF1NodeIds/SysConNodeId"))->getValue();
		m_eipThreadArg.eipSourceId = BposReadInt( XmlToAscii( tmp).c_str());
	}
	catch( XmlException &err)
	{
		Log( LOG_ERRORS, "\tNO EIP SOURCE NODE ID SPECIFIED!!!\n");
		m_eipThreadArg.eipSourceId = 0xFF;
	}

	// Read the EIP destination node IDs
	try
	{
		const XmlString& tmp = plcSetup->getChild( XML_T("DF1NodeIds/PlcNodeId"))->getValue();
		m_eipThreadArg.eipDestId = BposReadInt( XmlToAscii( tmp).c_str());
	}
	catch( XmlException &err)
	{
		Log( LOG_ERRORS, "\tNO EIP DESTINATION NODE ID SPECIFIED!!!\n");
		m_eipThreadArg.eipDestId = 0xFF;
	}
#endif 
...
/**
 * Launches the EIP serial comm thread (used to update PLC data arrays
 * via serial messages to and from the EIP PLC). Also opens the logical
 ...
 */
void PlcEip::LaunchCommThread( const std::string &portName) throw( BepException)
{
	std::string errStr;
	int         status;

	Log( LOG_FN_ENTRY, "Enter PlcEip::LaunchCommThread()\n");

	// Don't need to open the serial port here. The comm thread calls for the EIP connection to be opened

	try
	{

		pthread_barrier_init( &m_eipThreadArg.barrier, NULL, 2);
		m_eipThreadArg.parent = this;
		m_eipThreadArg.fromPlc = m_fromPlc; m_eipThreadArg.fromCount = m_fromPlcCount;
		m_eipThreadArg.toPlc = m_toPlc;     m_eipThreadArg.toCount = m_toPlcCount;
		m_eipThreadArg.plc = &m_plc;
		m_eipThreadArg.logger = (ILogger *)this;
		m_eipThreadArg.rwLock = (pthread_rwlock_t*)&m_plcLock;
		m_eipThreadArg.nsecLoopTime = mSEC_nSEC( m_commScanTime);
		m_eipThreadArg.threadStop = false;
		m_eipThreadArg.ipAddress = portName;

		pthread_create( &m_eipThreadArg.threadId, NULL, &PlcEip::EipCommThread, (void*)&m_eipThreadArg);
		// Wait for serial comm thread to start
		status = pthread_barrier_wait( &m_eipThreadArg.barrier);
		if( (status != BARRIER_SERIAL_THREAD) && (status != 0))
		{
			errStr = "EIP comm thread init error: ";
			errStr += strerror( status);
			throw( BepException( errStr));
		}
		Log( LOG_DEV_DATA, "EIP comm thread launched with thread id of %d\n",
			 m_eipThreadArg.threadId);
		// Wait for the first scan to complete
		delay( m_commScanTime*2);
	}
	catch( BepException &err)
	{
		Log( LOG_ERRORS, "\tError initializing EIP PLC DLL: %s\n", err.GetReason());
		throw;
	}


	Log( LOG_FN_ENTRY, "Exit PlcEip::LaunchCommThread()\n");
}
 ...
 */
void PlcEip::ReadInputConfiguration( const XmlNodeMap &inputNodes)
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::ReadInputConfiguration()\n");

	try
	{
		const XmlNode *addressNode = inputNodes.getNode( "Address");
		m_eipThreadArg.readAddress = XmlToAscii( addressNode->getValue());
	}
	catch( XmlException &err)
	{
		m_eipThreadArg.readAddress = "$N17";
	}

	Log( LOG_DEV_DATA, "Input Address: %s\n", m_eipThreadArg.readAddress.c_str());

	Log( LOG_FN_ENTRY, "Exit PlcEip::ReadInputConfiguration()\n");
}
 ...
 */
void PlcEip::ReadOutputConfiguration( const XmlNodeMap &outputNodes)
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::ReadOutputConfiguration()\n");

	// Check for output channel PLC address
	try
	{
		const XmlNode *addressNode = outputNodes.getNode( "Address");
		m_eipThreadArg.writeAddress = XmlToAscii( addressNode->getValue());
	}
	catch( XmlException &err)
	{
		m_eipThreadArg.writeAddress = "$N16";
	}
	Log( LOG_DEV_DATA, "Output Address: %s\n", m_eipThreadArg.writeAddress.c_str());

	Log( LOG_FN_ENTRY, "Exit PlcEip::ReadOutputConfiguration()\n");
}
...
void PlcEip::Close()
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::Close\n");

	// If comm thread is running
	if( m_eipThreadArg.threadId != -1)
	{
		...
		m_eipThreadArg.threadStop = true;
		MsgSendPulse( m_eipThreadArg.coId, 62, SYSTEM_PULSE, TERMINATE_PULSE);

		// Wait for the thread to die
		pthread_join( m_eipThreadArg.threadId, NULL);
	...
	Log( LOG_FN_ENTRY, "Exit PlcEip::Close\n");
}
 ...
 */
uint32_t PlcEip::ReadChannel( uint32_t channelIdx, portDir_t portDir)
{
	uint32_t            retVal;
 ...
 *         0 if port is "off"
 */
uint32_t PlcEip::ReadPort( uint32_t channelIdx, uint16_t portIdx, uint16_t logicLvl, portDir_t portDir)
{
	uint32_t    retVal;
 ...
 */
void PlcEip::WriteChannel( uint32_t channelIdx, uint32_t channelVal)
{
	int errVal;
	...
	if( m_toPlc != NULL)
	{
		...
		{
			ThrowOsException( errVal, "Error attaining write lock in PlcEip::WriteChannel");
		}
	}
	else
	{
 ...
 */
void PlcEip::WritePort( uint32_t channelIdx, uint16_t portIdx, uint16_t logicLvl, uint32_t portVal)
{
 ...
 * @return this
 */
PlcEip& PlcEip::operator=( const PlcEip& rhs)
{
	if( this != &rhs)
	{
		Close();
		...
		m_eipPortName = rhs.m_eipPortName;
 ...
 */
void PlcEip::CreateDataArrays()
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::CreateDataArrays()\n");
	...
	Log( LOG_FN_ENTRY, "Exit PlcEip::CreateDataArrays()\n");
}

/**
 * De-allocates memory for the data arrays used to hold the PLC data
 */
void PlcEip::DestroyDataArrays()
{
	Log( LOG_FN_ENTRY, "Enter PlcEip::DestroyDataArrays()\n");

	if( m_fromPlc != NULL)	delete[] m_fromPlc;
	if( m_toPlc != NULL)	delete[] m_toPlc;

	m_fromPlc = m_toPlc = NULL;
	Log( LOG_FN_ENTRY, "Exit PlcEip::DestroyDataArrays()\n");
}
 ...
 */
void PlcEip::ResolvePathName( string &pathToResolve)
{
	// Make a copy of the original
	string::size_type   envStart, envEnd;
	char                *envValue;

	Log( LOG_FN_ENTRY, "Enter PlcEip::ResolvePathName( %s)\n", pathToResolve.c_str());
	...
	Log( LOG_FN_ENTRY, "Exit PlcEip::ResolvePathName( %s)\n", pathToResolve.c_str());
}

INT32 PlcEip::SystemWrite(const std::string &tag, const std::string &value)
{
	...
	{
		Log(LOG_ERRORS, "PlcEip::SystemWrite %s Error: %s\n", tag.c_str(), e.what());
	}
...
/**
 * Thread entry point used to perform EIP serial communication
 *
 * @param threadArg Pointer to a EipThreadArg_t data structure
 */
void* PlcEip::EipCommThread( void *threadArg)
{
	EipThreadArg_t      *arg = (EipThreadArg_t*)threadArg;
	int                 chId, lockStat;
	PlcEip              *parentPlc = arg->parent;
	timer_t             timerId;
	...
	// Initialize
	logger->Log( LOG_FN_ENTRY, "Enter PlcEip::EipCommThread()\n");
	...
	if( (timerId=BposCreateTimerWithPulse( &chId, &arg->coId, SYSTEM_PULSE+1)) != -1)
	{
		...
		if( (lockStat == BARRIER_SERIAL_THREAD) || (lockStat == 0))
		{
			...
			while( false == arg->threadStop)
			{
				...
				if( (lockStat=MsgReceivePulse( chId, &timerPulse, sizeof( timerPulse), NULL))  == 0)
				{
					...
					case SYSTEM_PULSE+1:
						...
						if( serialPlc->Read(fromPlc, fromCount, arg->ipAddress, arg->readAddress.c_str()) == true)
						{
							...
							{
								logger->Log( LOG_ERRORS, "\tEIPCommThread Error locking rwlock for read: %s\n",
											 strerror( lockStat));
							}
							...
							if( faultSet & 0x01)
							{
								logger->Log( LOG_ERRORS, "\tEIPCommThread clearing PLCSerialReadFault\n");
						...
						{
							logger->Log( LOG_ERRORS, "\tEIPCommThread setting PLCSerialReadFault\n");
						...
						if( (lockStat=pthread_rwlock_wrlock( rwLock)) == EOK)
						{
							...
							if( serialPlc->Write(toPlc, toCount, arg->ipAddress, arg->writeAddress.c_str()) == true)
							{
								...
								if( faultSet & 0x02)
								{
									logger->Log( LOG_ERRORS, "\tEIPCommThread clearing PLCSerialWriteFault\n");
							...
							{
								logger->Log( LOG_ERRORS, "\tEIPCommThread setting PLCSerialWriteFault\n");
						...
						{
							logger->Log( LOG_ERRORS, "\tEIPCommThread Error locking rwlock for write: %s\n",
										 strerror( lockStat));
						}
						break;
					case SYSTEM_PULSE:
						logger->Log( LOG_DEV_DATA, "EIP Comm thread received SYSTEM_PULSE %d\n",
									 timerPulse.value.sival_int);
						if( timerPulse.value.sival_int == TERMINATE_PULSE)	arg->threadStop = true;

						break;
					default:
						logger->Log( LOG_DEV_DATA, "EIP Comm thread received unexpected pulse: %d,%d\n",
				...
				{
					logger->Log( LOG_ERRORS, "\tEIP Comm thread MsgReceive error: %s\n",
								 strerror( errno));
				}
			}
			logger->Log( LOG_DEV_DATA, "EIP Comm thread received TERMINATE_PULSE...ending\n");
		}
		else
		{
	...
	logger->Log( LOG_FN_ENTRY, "Exit PlcEip::EipCommThread()\n");
