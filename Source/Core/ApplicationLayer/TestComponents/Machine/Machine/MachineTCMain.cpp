//*************************************************************************
// FILE:
//    $Header: /Core/ApplicationLayer/TestComponents/Machine/Machine/MachineTCMain.cpp 1     12/21/03 6:33p Cward $
//
// FILE DESCRIPTION:
...
//
// LOG:
//    $Log: /Core/ApplicationLayer/TestComponents/Machine/Machine/MachineTCMain.cpp $
// 
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		component = new MachineTC();    // create a machine component component
	...
	{
		printf("MachineTC -%s: XmlExcpetion: %s", clp.GetConfigFile().c_str(), XmlErr.what());
	}
	catch(BepException &BepErr)
	{
		printf("MachineTC -%s: BepExcpetion: %s", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("MachineTC -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}

	if(component != NULL)	delete component;	// delete the component object
	component = NULL;
	if(clp.IsDebugOn())
		printf("MachineTC (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
