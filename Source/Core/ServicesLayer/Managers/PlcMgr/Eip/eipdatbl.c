...
DataTableEntry* dataTableGetEntry( REQUEST* pRequest )
{
    ...
    memcpy( tag, (UINT8*)data, iTagSize );
    tag[iTagSize] = 0;
    ...
    for( pDataTableEntry = 0; pDataTableEntry < &dataTableEntries[DATA_TABLE_ENTRIES]; pDataTableEntry++ )
    {
        /* Check if the last char is a pad byte */
        if ( !strcmp(pDataTableEntry->Tag, tag) )
...
void dataTableHandleReadRequest( REQUEST* pRequest, DataTableEntry* pDataTableEntry )
{
    ...
    switch( pDataTableEntry->elementType )
    {
        case typeINT:
            iType = INT_TYPE;
            nLen = iNbrOfElements * sizeof(UINT16);            
            break;

        case typeDINT:
            iType = DINT_TYPE;
            nLen = iNbrOfElements * sizeof(UINT32);
            break;
    ...
    EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, NULL, (UINT16)(nLen+sizeof(UINT16)) );        
    ...
    UINT16_SET( pRequest->cfg.objectCfg.pData, iType );
    
    assemblyGetCombinedData( (UINT8*)(pRequest->cfg.objectCfg.pData+sizeof(UINT16)), (INT32)pDataTableEntry->nOffset, (UINT16)nLen );    
}
...
void dataTableHandleWriteRequest( REQUEST* pRequest, DataTableEntry* pDataTableEntry )
{
    INT32  nLen = ( pDataTableEntry->iSize > (pRequest->cfg.objectCfg.iDataSize-sizeof(UINT32)) ) ?
        (pRequest->cfg.objectCfg.iDataSize-sizeof(UINT32)) : pDataTableEntry->iSize;                   

    /* Adjust for the tag type and number of elements */
    assemblySetCombinedData( (UINT8*)(pRequest->cfg.objectCfg.pData+sizeof(UINT32)), (INT32)pDataTableEntry->nOffset, (UINT16)nLen );
}
