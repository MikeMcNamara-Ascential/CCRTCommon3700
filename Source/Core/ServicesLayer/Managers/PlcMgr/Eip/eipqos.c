...
/* local functions */
void qosParseClassRequest( REQUEST* pRequest );
void qosSendClassAttrSingle( REQUEST* pRequest );
void qosParseInstanceRequest( REQUEST* pRequest, UINT32 iInstance );
void qosSendInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
void qosSetInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
...
{    
    ...
    switch( iAttribute )
    {
        ...
        case QOS_CLASS_ATTR_MAX_INSTANCE:
            iVal = (UINT16)ENCAP_TO_HS(gnClaimedHostIPAddrCount);
            EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, (UINT8*)&iVal, sizeof(UINT16) );        
            break;
