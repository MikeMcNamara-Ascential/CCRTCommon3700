...
/*******************
 include files
 ******************/
#include "LifanSystemMonitor.h"

//-----------------------------------------------------------------------------
LifanControlData::LifanControlData() : ControlData(), rollDrivesEnabled(false)
{
}

//-----------------------------------------------------------------------------
LifanControlData::~LifanControlData()
{
}

//-----------------------------------------------------------------------------
LifanSystemMonitor::LifanSystemMonitor() : SystemMonitor(), m_rollerCount(4),
...
//-----------------------------------------------------------------------------
LifanSystemMonitor::LifanSystemMonitor( int argc, char *argv[]) : SystemMonitor( argc, argv),
...
//-----------------------------------------------------------------------------
LifanSystemMonitor::~LifanSystemMonitor()
{

}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::Initialize(const XmlNode *document)
{
    Log( LOG_FN_ENTRY, "Enter LifanSystemMonitor Initialize()\n");
    ...
    Log( LOG_FN_ENTRY, "Exit Lifan Initialize()\n");
}

//-----------------------------------------------------------------------------
const std::string LifanSystemMonitor::Publish(const XmlNode *node)
...
//-----------------------------------------------------------------------------
const std::string LifanSystemMonitor::Register(void)
{
...
//-----------------------------------------------------------------------------
void LifanSystemMonitor::Run(volatile bool *terminateFlag/*=NULL*/)
{
    Log( LOG_FN_ENTRY, "LifanSystemMonitor Enter Run()\n");
    ...
    Log( LOG_FN_ENTRY, "LifanSystemMonitor Exit Run()\n");
}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::GetCurrentControlData( ControlData *ctrl)
{
    LifanControlData *jianglingCtrl = (LifanControlData *)ctrl;

    Log( LOG_FN_ENTRY, "Enter Lifan GetCurrentControlData()\n");
    ...
    Log( LOG_FN_ENTRY, "Exit Lifan GetCurrentControlData()\n");
}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::CheckTesting( ControlData *ctrl)
{
    LifanControlData *jianglingCtrl = (LifanControlData *)ctrl;
    ...
    if((!ctrl->machineFault)  && (!ctrl->calSwitch))
    {
        ...
        /*else if(ctrl->vehiclePresent && ctrl->rollsUp &&
                !m_startVehicleTestFlag && (ctrl->vehVinReadStatus == VALID_VEHICLE_VIN))
        {
            // Remove the "Enter VIN" prompt
        ...
        {                              // if the cable is still connected, prompt to operator to remove the cable
            if(ctrl->cableConnect)
            {
                ...
                Log(LOG_DEV_DATA, "Prompting to Disconnect Cable\n");
            }
        ...
        else if(!ctrl->vehiclePresent &&  (ctrl->vehVinReadStatus == VALID_VEHICLE_VIN) && !ctrl->startTestPb && !m_startVehicleTestFlag)
        {
            DisplayPrompt(1,"DriveOn");
        }
        else if(ctrl->vehiclePresent && !ctrl->startTestPb && !m_startVehicleTestFlag)
        {
            // if we don't have a valid vin
            if(ctrl->vehVinReadStatus != VALID_VEHICLE_VIN)
            {
                DisplayPrompt(1,"EnterVIN");
                Log(LOG_DEV_DATA, "Prompting to Enter VIN\n");
            }
            // if we have a valid vin and the vehicle build hasn't been loaded
            if((ctrl->vehVinReadStatus == VALID_VEHICLE_VIN))
            {
                // Remove the "Enter VIN" prompt
                RemovePrompt(1, "EnterVIN");

                // if the rolls aren't up, display the prompt
                if(!ctrl->rollsUp)
                {
                    DisplayPrompt(1, "RaiseRetainers");
                    Log(LOG_DEV_DATA, "Prompting to Raise Retainers\n");
                }
                else
                {
                    RemovePrompt(1, "RaiseRetainers");
                    DisplayPrompt(1, "PressButtonToStartTest");
                    Log(LOG_DEV_DATA, "Prompting to Push Button\n");
                }
            }
        }
    ...
    if((ctrl->rollsDown) && !(m_oldCtrl->rollsDown))
    {
        ...
        if(m_clearResultsOnRollsDown == true)
        {
            ...
            Log(LOG_DEV_DATA, "Prompting to Drive Off\n");
        }
    ...
    Log( LOG_FN_ENTRY, "Exit Lifan CheckTesting()\n");
}

//-----------------------------------------------------------------------------
INT32 LifanSystemMonitor::GetWheelSpeeds(float wheelSpeed[])
{
    INT32 testStatus = BEP_STATUS_FAILURE;    // overall status

    Log(LOG_FN_ENTRY, "Lifan GetWheelSpeeds():\n");
...
//-----------------------------------------------------------------------------
const INT32 LifanSystemMonitor::HandlePulse(const INT32 code, const INT32 value)
{
    INT32 retVal = BEP_STATUS_SUCCESS;

    Log( LOG_FN_ENTRY, "Enter Lifan HandlePulse(%d, %d)\n", code, value);
    ...
    Log( LOG_FN_ENTRY, "Exit Lifan HandlePulse(%d, %d)\n", code, value);

    return( retVal);
}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::UpdateTestConditions( ControlData *ctrl)
{
    Log(LOG_FN_ENTRY, "Lifan UpdateTestConditions() entry\n");
    ...
    Log(LOG_FN_ENTRY, "Lifan UpdateTestConditions() exit\n");
}

//-----------------------------------------------------------------------------
INT32 LifanSystemMonitor::GetRollerCount( void)
{
    return(m_rollerCount);
}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::SetRollerCount( INT32 rollerCount)
{
    m_rollerCount = rollerCount;
}

//-----------------------------------------------------------------------------
ControlData* LifanSystemMonitor::ControlAllocate() const
{
    LifanControlData *control = new LifanControlData;

    return( (ControlData*)control);
}

//-----------------------------------------------------------------------------
void LifanSystemMonitor::ControlFree(ControlData* &controlData) const
{
    LifanControlData *control = (LifanControlData *)controlData;
    if( NULL != control)
    {
        delete control;
        control = NULL;
