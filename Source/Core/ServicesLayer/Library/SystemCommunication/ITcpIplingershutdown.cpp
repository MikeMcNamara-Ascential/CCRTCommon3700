...
ITcpIp::~ITcpIp(void)
{
	printf("ITcpIp::~ITcpIp(%d)\n", GetSock());
	if(IsDebugOn())	printf("ITcpIp::~ITcpIp(%d)\n", GetSock());
    printf("ITcpIp Calling CloseSocket from destructor\n");
	CloseSocket(GetSock());
...
void ITcpIp::Initialize(const XmlNode *config, const INT32 id /* =-1 */, const INT32 rid /* =-1 */)
{
	...
	{	// if this is not connecting to a currently open socket
        printf("ITcpIp Calling CloseSocket from id = -1 function\n");
...
{	// AF_INET - IPv4 Protocol (AF_INET6 - IPv6 Protocol)
	...
	INT16 sock = socket(AF_INET, SOCK_STREAM, 0);	// create a socket
	if(sock >= 0)
	{	// if a socket was opened
        ...
        if(setsockopt(sock, IPPROTO_TCP, SO_LINGER, &ling, sizeof(ling)) != -1)
		{
			if(IsDebugOn())	printf("Updated SO_LINGER status: %s\n", ((value == 0) ? "off" : "on"));
		}
		else printf("SO_LINGER could not be set in RTP\n");
#if 0
...
{
    printf("ITcpIp::CloseSocket(%d, %s) connected to: %d, should close %d\n", newValue, GetType().c_str(), GetSock(), GetCloseOnExit());
	INT32 counter = 10, stat = -1;
	...
	{	// If connected to a socket
		...
		{	// Close socket repeating if interrupted by signal
			...
			stat = close(GetSock());
            printf("ITcpIp::CloseSocket() returned %s\n", strerror(stat));
			pthread_testcancel();
		}
		while(SignalCheck(stat, EINTR, counter, IsDebugOn()) == false);	// Do over if interrupted
...
{
    printf("ITcpIp Calling CloseSocket from Close() function\n");
    CloseSocket(GetSock());
}
...
inline void ITcpIp::SetCloseOnExit(const bool close)
{
    m_closeOnExit = close;
}

inline const bool ITcpIp::GetCloseOnExit(void) const
{
    return(m_closeOnExit);
}

inline const std::string ITcpIp::GetIPAddress()
{
    if(IsDebugOn()) printf("GetIPAddress: %s\n", m_address.c_str());
    return(m_address);
}
inline void ITcpIp::SetIPAddress(const std::string &value)
{
    if(IsDebugOn()) printf("SetIPAddress: %s\n", value.c_str());
    if(!value.empty())  m_address = value;
    else                throw BepException("Invalid IP Address");
}
inline const INT16 ITcpIp::GetPort()
{
    if(IsDebugOn())  printf("GetPort %d\n", m_port);
    return(m_port);
}
inline void ITcpIp::SetPort(INT16 value)
{
...
inline void ITcpIp::SetPort(const std::string &value)
{
...
inline const INT32 ITcpIp::GetSock() const
{
    return((const INT16) GetId());
}
inline void ITcpIp::SetSock(const INT32 &value)
{
    if(IsDebugOn()) printf("ITcpIp::SetSock(%d)\n", value);
    if(value > 0)
    {   // If id is valid
        printf("ITcpIp Calling CloseSocket from setsock function\n");
        CloseSocket(value);
        SetId((const INT32) value);
    }
    else throw BepException("Invalid Socket Vaule: %d", value);
}
inline const struct sockaddr_in ITcpIp::GetServer()
{
    return(m_server);
}
inline void ITcpIp::SetServer(const struct sockaddr_in &value)
{
    m_server = value;
}
inline const std::string ITcpIp::GetFrameStart()
{
    return(m_frameStart);
}
inline void ITcpIp::SetFrameStart(const std::string &value)
{
    if(IsDebugOn()) printf("SetFrameStart: %s\n", value.c_str());
    m_frameStart = value;
}
inline const std::string ITcpIp::GetFrameEnd()
{
    return(m_frameEnd);
}
inline void ITcpIp::SetFrameEnd(const std::string &value)
{
    if(IsDebugOn()) printf("SetFrameEnd: %s\n", value.c_str());
    m_frameEnd = value;
}
inline void ITcpIp::SetBackLog(INT32 backlog)
{
    if(IsDebugOn()) printf("SetBacklog: %d\n", backlog);
    if(backlog > 5) m_backlog = backlog;
    else m_backlog = 5;
}
inline const INT32 ITcpIp::GetBacklog(void)
{
    return(m_backlog);
}

inline const INT32& ITcpIp::GetBufferSize(void)
{
	return m_bufferSize;
}

inline void ITcpIp::SetBufferSize(const INT32 &bufferSize)
