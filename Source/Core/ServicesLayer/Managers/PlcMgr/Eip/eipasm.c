...
**  Local Functions
***************************************************************************/
void assemblyInitialize( ASSEMBLY* pAssembly );

void assemblyParseClassRequest( REQUEST* pRequest );
void assemblyParseInstanceRequest( REQUEST* pRequest, UINT32 iInstance );
void assemblySendInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
void assemblySetInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
void assemblySendClassAttrAll( REQUEST* pRequest );
void assemblySendClassAttrSingle( REQUEST* pRequest );
void assemblySendMember( REQUEST* pRequest, UINT32 iInstance );
void assemblySetMember( REQUEST* pRequest, UINT32 iInstance );
void assemblyCreateDynamic( REQUEST* pRequest );
void assemblyDeleteDynamic( REQUEST* pRequest, UINT32 iInstance );

void assemblyGetBitOffsetData( UINT8* pSrc, UINT8* pDest, UINT16 iSrcOffset, UINT16 iLength );
void assemblySetBitOffsetData( UINT8* pSrc, UINT8* pDest, UINT16 iDestOffset, UINT16 iLength );

void assemblyIncrementSequenceCount(UINT32 iInstance);
...
UINT16 assemblyOptionalAttributes[ASSEMBLY_ATTR_OPTIONAL_COUNT] = {ASSEMBLY_INSTANCE_SIZE};
...
INT32 assemblyNew( UINT32 iInstance, INT32  nOffset, UINT16 iSize, UINT16 iType, INT8* szAssemblyName )
{
    ...
    if ( gpnAssemblies >= &gAssemblies[MAX_ASSEMBLIES] )    
        return ERR_ASSEMBLY_OBJECTS_LIMIT_REACHED;                                /* Assembly array limit has been reached */

    if ((iInstance == HEARTBEAT_CONN_POINT) || 
        (iInstance == LISTEN_ONLY_CONN_POINT)
	...
	if ( nOffset == (INT32)INVALID_OFFSET )                        /* Assign offset automatically */
    {    
        for ( i = (nUpperBound-iSize); i >= 0; i -= 2 )    /* Assign at the end of the buffer since the beginning might be assigned manually */
        {            
            for( pAssembly = gAssemblies; pAssembly < gpnAssemblies; pAssembly++ )        /* Find if assembly already exist */
            {                
    ...
    else                                                    /* Offset specified by the client */
    {
        if ( nOffset < 0 || (nOffset+iSize) > ASSEMBLY_SIZE )
            return ERR_ASSEMBLY_SPECIFIED_OFFSET_INVALID;
        
        for( pAssembly = gAssemblies; pAssembly < gpnAssemblies; pAssembly++ )    /* Check that it does not overlap with any of the already configured assemblies */
        {                
...
{    
    ...
    for( pAssembly = gAssemblies; pAssembly < gpnAssemblies; pAssembly++ )        /* Find our request */
    {
        if ( pAssembly->iInstance == iInstance )
            return pAssembly;
    }
...
UINT16 assemblyGetMemberData( UINT32 iInstance, UINT32 iMember, UINT8* pData, UINT16 iSize ) 
{
    ...
    if ( (pAssembly = assemblyGetByInstance(iInstance)) == NULL )
        return 0;                                                        /* Assembly instance not found */

    if ( pAssembly == NULL || iMember == INVALID_MEMBER ) 
        return 0;                                                /* Assembly instance not found or member Id not valid */

    pMember = assemblyGetMember(pAssembly, iMember);

    if (pMember == NULL)
        return 0;                                                /* Member Id not found */

    iBitSize = pMember->iSize;
    
    iSize *= 8;                                                    /* Convert to bits */
    if ( iBitSize > iSize )
        iBitSize = iSize;

    iByteSize = (UINT16)((iBitSize+7)/8);                /* Number of bytes used in pData */ 
    
    assemblyGetBitOffsetData(&gAssemblyBuffer[pAssembly->nOffset], pData, pMember->iOffset, iBitSize );

    return iByteSize;                                    /* Return number of bytes used in pData */
}
...
void assemblyParseClassInstanceRqst( REQUEST* pRequest )
{
    ...
    if ( iTagSize == 0 )        /* Instance is required even for a class attribute */
    {
...
void assemblyCreateDynamic( REQUEST* pRequest )
{
    ...
    if ( pRequest->cfg.objectCfg.iDataSize >= 2 )                            /* If new instance is specified use it. Otherwise assigned automatically from the vendor specific range. */
    {
...
#endif
        pAssembly = assemblyGetByInstance( iInstance );
        if ( pAssembly != NULL )                                /* Instance already created */        
        {
            pRequest->bGeneralError = ROUTER_ERROR_OBJECT_STATE_CONFLICT;
            EtIP_free( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize );            
            return; 
    ...
    else
    {
        ...
        if ( iInstance == 0 )                /* Object in a state where all dynamic instances are already used */
        {
            pRequest->bGeneralError = ROUTER_ERROR_OBJECT_STATE_CONFLICT;
            EtIP_free( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize );            
            return; 
...
/*--------------------------------------------------------------------------------
** assemblySendClassAttrAll( )
**
** GetAttributeaLL request has been received for the Assembly object class
**--------------------------------------------------------------------------------
*/

void assemblySendClassAttrAll( REQUEST* pRequest )
{
    ASSEMBLY_CLASS_ATTR attr;
    ASSEMBLY*     pAssembly;
    UINT16        iMaxInstance = 0;


    attr.iRevision = ENCAP_TO_HS(ASSEMBLY_CLASS_REVISION);
    attr.iMaxClassAttr = ENCAP_TO_HS(ASSEMBLY_CLASS_ATTR_MAX_INST_ATTR);
    attr.iMaxInstanceAttr = ENCAP_TO_HS(ASSEMBLY_INSTANCE_SIZE);
#if defined ET_IP_SCANNER && !defined (EIP_NO_CCO)
    attr.iNumInstances = (UINT16)ENCAP_TO_HS((gpnAssemblies-gAssemblies+NUM_ASSEMBLY_SCANNER_INPUT_INSTANCES+NUM_ASSEMBLY_SCANNER_OUTPUT_INSTANCES));
#else
   attr.iNumInstances = (UINT16)ENCAP_TO_HS((gpnAssemblies-gAssemblies));
#endif

	/* compute max assembly instance */
	for( pAssembly = gAssemblies; pAssembly < gpnAssemblies; pAssembly++ )        
	{
...
#endif
    attr.iMaxInstance = ENCAP_TO_HS(iMaxInstance);
...
void assemblySendClassAttrSingle( REQUEST* pRequest )
{
    ...
    if ( iTagSize == 0 )                                        /* Attribute is required */
    {
        pRequest->bGeneralError = ROUTER_ERROR_INVALID_DESTINATION;
        return; 
    }
...
void assemblyDeleteDynamic( REQUEST* pRequest, UINT32 iInstance )
{
    ...
    pAssembly = assemblyGetByInstance( iInstance );
    if ( pAssembly == NULL )                                /* Instance not found */        
    {
        pRequest->bGeneralError = ROUTER_ERROR_INVALID_DESTINATION;
        return; 
    }

    if ( pAssembly->iType & AssemblyStatic )                /* Only dynamic assemblies can be deleted */        
    {
        pRequest->bGeneralError = ROUTER_ERROR_BAD_SERVICE;
        return; 
    }
...
{    
	...
	{	
		...
		if (pAssembly->pnMembers == pAssembly->Members)                            /* Member list has not been set yet for this assembly */
		{
			if (pAssembly->iType & AssemblyStatic)
			{
		...
		if ( iTagSize == 0 )                                        /* Member is required */
		{    
			pRequest->bGeneralError = ROUTER_ERROR_INVALID_DESTINATION;
			EtIP_free( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize );            
			return; 
		...
		if (pMember == pAssembly->pnMembers)                            /* Exact match not found */
		{ 
			if ( iStartMember > iMaxMember )                            /* If specified member larger than the max member id - provide one last member in the list */
			{
				pMember = pAssembly->pnMembers - 1;
				iMembers = 1;
			}
			else                                                                    /* Otherwise return invalid member error */
			{
