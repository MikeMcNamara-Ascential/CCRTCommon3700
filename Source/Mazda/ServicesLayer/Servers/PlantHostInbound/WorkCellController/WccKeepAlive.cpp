//*************************************************************************
// FILE:
//    $Header: /CCRT-cmake/Source/DCX/ServicesLayer/Servers/PlantHostInbound/WorkCellController/WccKeepAlive.cpp 3     10/17/07 4:04p Jsemann $
//
// FILE DESCRIPTION:
...
//
// LOG:
//    $Log: /CCRT-cmake/Source/DCX/ServicesLayer/Servers/PlantHostInbound/WorkCellController/WccKeepAlive.cpp $
// 
// 3     10/17/07 4:04p Jsemann
// Added log statement to indicate task info could not be read.  In this
// situation the latest TaskMon and TaskMonTerm must be loaded on the
// plant machine in the /usr/local/bin/ directory.
// 
// 2     7/30/07 4:05p Jsemann
// -  added check return from ReadTaskInfo - if -1 log error (if task
// monitor is not up to date this function does not work)
// 
// 1     10/26/06 1:22p Bbarrett
// Intro to Cmake.
// 
// 8     8/30/06 4:10p Derickso
// Added delay prior to registering port driver to ensure port is up.
// 
...
AtomicThread(), m_machineNumber(machineNumber), m_wcc(NULL)
...
{	// Build the keep alive string
	...
	{
		m_wcc->Log(LOG_ERRORS, "ReconncetDelay not specified, using 1s: %s", excpt.GetReason());
...
{	// Keep sending keep alives until the end of time, or we get terminated
	...
	// Wait for the port to connect to the remote system
	WaitForWorkcellController();
		...
		if(iter != m_errorCodeDefs.end())
		{
			...
			{	// Error code was found, perform the neccessary actions.
				...
				{  // Current bit is set, process the error data
                    ...
                    //try setting PLC bit if specified
                    try 
                    {
                        m_wcc->m_broker->Write(iter->second->getAttribute("PLCBitTag")->getValue(), std::string("1"), response, true);
                    }
                    catch(XmlException &excpt)
                    {
                        m_wcc->Log(LOG_ERRORS, "No PLC bit node for %s configured: %s", errorTag.c_str(), excpt.GetReason());
                    }
				}
				else
				{
                    ...
                    //try clearing PLC bit if specified
                    try 
                    {
                        m_wcc->m_broker->Write(iter->second->getAttribute("PLCBitTag")->getValue(), std::string("0"), response, true);
                    }
                    catch(XmlException &excpt)
                    {
                        m_wcc->Log(LOG_ERRORS, "No PLC bit node for %s configured: %s", errorTag.c_str(), excpt.GetReason());
...
BEP_STATUS_TYPE WccKeepAlive::SendKeepAliveMessage(void)
{
	...
	{	// Send the message to the WCC after clearing the buffer
		...
		{	// Tell threads to ignore any incoming data
			...
			if(BEP_STATUS_SUCCESS == messageStatus)
			{
				...
				// If WCC has been shutdown, wait for 1 second before attempting to talk to it again
			...
			m_wcc->Log(LOG_DEV_DATA, "WccKeepAlive: Unlocking comm port for others");
	...
	} while((messageStatus == BEP_STATUS_TIMEOUT) && (m_wcc->GetStatus().compare(BEP_TERMINATE)));
...
BEP_STATUS_TYPE WccKeepAlive::GetKeepAliveResponse(void)
{
	...
	{	// No data received, set as disconnected
		...
		m_wcc->Log(LOG_ERRORS, "Lost comms with WCC, attempt to reconnect.");
	}
	else
	{
...
void WccKeepAlive::WaitForWorkcellController(void)
{
	TaskMonTask_t wccTask;
	do
	{   // Get the state of the WCC port
		if (ITaskMon::ReadTaskInfo(wccTask, "Wcc", TASKMON_CLASS_SERIAL) != -1)
		{
		...
		{
				m_wcc->Log(LOG_ERRORS, "Error Task already in run state\n");
			}
		...
		{
			m_wcc->Log(LOG_ERRORS, "Unable to read task info - Install latest TaskMon / TaskMonTerm to /usr/local/bin dir\n");
		}
		// Keep looking until state is run and we are not terminated
	} while((wccTask.m_taskState != TASKSTATE_RUN) && m_wcc->GetStatus().compare(BEP_TERMINATE));
}

void WccKeepAlive::SetReconnectDelay(const INT32 &delay)
{
	m_reconnectDelay = delay;
}

const INT32& WccKeepAlive::GetReconnectDelay(void)
{
