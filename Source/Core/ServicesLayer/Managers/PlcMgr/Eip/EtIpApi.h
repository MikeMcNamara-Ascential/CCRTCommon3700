...
#define ASSEMBLY_SCANNER_OUTPUT_INSTANCE            (ASSEMBLY_SCANNER_INPUT_INSTANCE+NUM_ASSEMBLY_SCANNER_INPUT_INSTANCES)

#define INPUT_SCANNER_ASSEMBLY_SIZE                 65481
#define OUTPUT_SCANNER_ASSEMBLY_SIZE                65481
#endif

#define HEARTBEAT_CONN_POINT    0xfe				/* Connection point to indicate there is no data transferred, just the heartbeat (0 data length) message */
#define LISTEN_ONLY_CONN_POINT    0xff				/* Connection point to indicate  a Listen Only behaviour in the O->T direction. There is no data transferred in the O->T direction, just the listen only heartbeat (0 data length) message */
...
#define NM_NETWORK_LED_CHANGE						  109	/* Network LED status has changed.  Second parameter is the new LED state */
...
typedef struct tagEtIPInternalConnectionConfig
{
	...
	UINT32  lSocket;					 /* Socket for producing/consuming data */
...
typedef void LogEventCallbackType( INT32 nEvent, INT32 nParam1, INT32 nParam2 );
/* Callback function to be called if logging messages to the client app */
typedef void LogMessageCallbackType(UINT8 iSeverity, UINT16 iType, UINT32 lSrcIp, UINT32 lDstIp, void *pData, UINT16 iLen );
...
{
	UINT16	iMaxActiveIOConnections;					/* The maximum number of class1 (I/O) connections open at one time */
	UINT16	iNumCurrentIOConnections;					/* Current number of class1 (I/O) connections */
	UINT16	iMaxActiveExplicitConnections;				/* The maximum number of class3 (explicit) connections open at one time */
	UINT16	iNumCurrentExplicitConnections;				/* Current number of class3 (explicit) connections */
