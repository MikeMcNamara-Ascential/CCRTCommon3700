...
/* local functions */
void tcpipInitConfig(INT32 iIndex);
void tcpipGetMcastBaseAddr(TCPIP_INTERFACE_INSTANCE_ATTRIBUTES* pTcpipConfig);
BOOL tcpipIsValidIPAddress(UINT32 lIPAddress);
BOOL tcpipIsValidSubnetMask(UINT32 lIPAddress);
void tcpipUpdateIPAddress(TCPIP_INTERFACE_INSTANCE_ATTRIBUTES* pTcpipConfig, UINT32 lNewIPAddress);
void tcpipParseClassRequest( REQUEST* pRequest );
void tcpipSendClassAttrAll( REQUEST* pRequest );
void tcpipSendClassAttrSingle( REQUEST* pRequest );
void tcpipParseInstanceRequest( REQUEST* pRequest, UINT32 iInstance );
void tcpipSendInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance );
void tcpipSendInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );
void tcpipSetInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance );
void tcpipSetInstanceAttrSingle( REQUEST* pRequest, UINT32 iInstance );

TCPIP_INTERFACE_INSTANCE_ATTRIBUTES gsTcpIpInstAttr[MAX_HOST_ADDRESSES];

UINT16 tcpipOptionalAttributes[TCPIP_ATTR_OPT_COUNT] = {TCPIP_INST_ATTR_TTL_VALUE, TCPIP_INST_ATTR_MCAST_CONFIG
#ifdef EIP_ACD
...
void tcpipInit(void)
{        
    gnClaimedHostIPAddrCount = 0;     /* Init TCP/IP object */
}	
...
**---------------------------------------------------------------------------
*/
void tcpipSocketCleanup(TCPIP_INTERFACE_INSTANCE_ATTRIBUTES* pTcpipConfig)
{
    SOCKET* pSocket;
...
void tcpipSocketCleanupAll(void)
{
         ...
         i++, pTcpipConfig++)
    {
     ...
     platformSocketLibCleanup();        /* Unload socket library */
}
...
TCPIP_INTERFACE_INSTANCE_ATTRIBUTES* tcpipFindClaimedHostAddress( UINT32 lHostIPAddress )
{
	...
	DumpStr2(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "tcpipFindClaimedHostAddress(lHostIPAddress: %d [%X]) - Enter",
			 lHostIPAddress, lHostIPAddress);

	DumpStr1(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "Claimed host addresses: %d", gnClaimedHostIPAddrCount);
    for( i = 0, pTcpIp = gsTcpIpInstAttr; 
         i < gnClaimedHostIPAddrCount; 
         i++, pTcpIp++)
    {
		DumpStr4(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "Checking %d [%X] against param { %d [%X] }", 
				 pTcpIp->InterfaceConfig.lIpAddr, pTcpIp->InterfaceConfig.lIpAddr, lHostIPAddress, lHostIPAddress);
...
{    
	...
	DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "tcpipAddClaimedIPAddress() - Adding claimed IP Address %d [%x]",
			 lAddress, lAddress);
	...
	DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Getting Tcp/Ip Config data");
	...
	DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Getting Tcp/Ip multi-cast base address");
    tcpipGetMcastBaseAddr(pTcpIp);

    /* Setup the listening socket */
	DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Setting up the listening socket");
	...
	if (pTcpIp->lPassiveWaitSocket == (SOCKET)INVALID_SOCKET)
		return FALSE;
	DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "listening socket: %d", pTcpIp->lPassiveWaitSocket);
	...
	DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0,0, "Setup broadcast socket: %d", pTcpIp->lBroadcastSocket);
	...
	DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Claimed host IP Address count: %d", gnClaimedHostIPAddrCount);
    return TRUE;
}
...
BOOL tcpipRemoveClaimedIPAddress(UINT32 lAddress)
{
    ...
    for( pConnection = gpnConnections-1; pConnection >= gConnections; pConnection-- )
    {
		...
		{
			if (pConnection->cfg.lIPAddress == lAddress)
				connectionRemove(pConnection, FALSE);
		}
	}
...
BOOL tcpipIsValidIPAddress(UINT32 lIPAddress)
{
	if (((lIPAddress >= 0x00000000) && (lIPAddress < 0x7F000000)) || /* Class A */
		((lIPAddress > 0x80000000) && (lIPAddress < 0xBFFFFFFF)) || /* Class B */
		((lIPAddress > 0xC0000000) && (lIPAddress < 0xDFFFFFFF)))	  /* Class C */
	{
		return TRUE;
	}
...
**---------------------------------------------------------------------------
*/
BOOL tcpipIsValidSubnetMask(UINT32 subnetMask)
{
...
void tcpipSendClassAttrAll( REQUEST* pRequest )
{
	...
	pData += sizeof(UINT16);
#ifdef EIP_QUICK_CONNECT
	UINT16_SET(pData, TCPIP_INST_ATTR_QUICK_CONNECT);
#elif defined EIP_ACD
	UINT16_SET(pData, TCPIP_INST_ATTR_ACD_LAST_CONFLICT);
#else
	UINT16_SET(pData, TCPIP_INST_ATTR_MCAST_CONFIG);
#endif
}
...
#ifdef EIP_QUICK_CONNECT
            iVal = ENCAP_TO_HS(TCPIP_INST_ATTR_QUICK_CONNECT)
#elif defined EIP_ACD
            iVal = ENCAP_TO_HS(TCPIP_INST_ATTR_ACD_LAST_CONFLICT);
#else
            iVal = ENCAP_TO_HS(TCPIP_INST_ATTR_MCAST_CONFIG);
#endif
            EtIP_realloc( &pRequest->cfg.objectCfg.pData, &pRequest->cfg.objectCfg.iDataSize, (UINT8*)&iVal, sizeof(UINT16) );            
			break;
         default:
...
void tcpipSendInstanceAttrAll( REQUEST* pRequest, UINT32 iInstance )
{
    ...
    UINT16  iHostNameLen, iDomainNameLen;
	UINT32 lVal;
...
#ifdef EIP_ACD
		+ (sizeof(UINT8)*2)+MAC_ADDR_LENGTH+TCPIP_MAX_ARP_PDU_LENGTH
#endif
#ifdef EIP_QUICK_CONNECT
		+ sizeof(UINT8)
#endif
	...
	**************************************************************/
	...
	/* Attribute 7 is not implemented, so leave 0s */
	memset(pData, 0, 6);
	pData += 6;
...
#elif EIP_QUICK_CONNECT
	/* Need to pad ACD attributes if quick connect is supported */
...
{    
    UINT8* pData;
    UINT32 lVal;
    UINT16 iDataSize, iLen, iTagSize;
    UINT32 iAttribute = (UINT32)pduGetTagByType( &pRequest->cfg.objectCfg.PDU, TAG_TYPE_ATTRIBUTE, 0, &iTagSize );
    TCPIP_INTERFACE_INSTANCE_ATTRIBUTES* pTcpipInterface = &gsTcpIpInstAttr[iInstance-1];
...
void tcpipReset(UINT8 bReset)
{
		 ...
		 i++, pTcpipConfig++)
	{
		if (bReset == 1)
		{
			...
			/* Save the data */
			platformSetTcpIpCfgData((1 << TCPIP_INST_ATTR_TTL_VALUE) | (1 << TCPIP_INST_ATTR_MCAST_CONFIG),
