...
//-------------------------------------------------------------------------------------------------
MazdaMachineTC::MazdaMachineTC() : MachineTC() {   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
MazdaMachineTC::~MazdaMachineTC() {   // Nothing special to do here
}
...
const string MazdaMachineTC::CommandTestStep(const string &value) {
    ...
    if (BEP_STATUS_SUCCESS == StatusCheck())
    {
        try
        {
            if (!GetTestStepName().compare("WaitForAcceleration"))
                testResult = WaitToStart();
            else if (!GetTestStepName().compare("RainLightSensorVerification"))
                testResult = TestStepRainLightSensorVerification();
            else if (!GetTestStepName().compare("ReportSideSlipResults"))
                testResult = ReportSideSlipResults();
            ...
            else if (!GetTestStepName().compare("StartTest"))
                testResult = StartTest();
            else if (!GetTestStepName().compare("StopMaxSpeedObservation"))
                testResult = StopMaxAxleSpeedObservation();
            else if (!GetTestStepName().compare("StopOdometerTest"))
                testResult = StopOdometerTest();
            else if (!GetTestStepName().compare("VehicleDisconnect"))
                testResult = VehicleDisconnect();
            else if (!GetTestStepName().compare("VehicleTestSetup"))
            ...
            else if (!GetTestStepName().compare("FinishUp"))
            {
                ...
                testResult = MachineTC::CommandTestStep(value);
...
const string MazdaMachineTC::TestStepSpeedometer(const string &value) {
    ...
    */
    float minSpeed = GetVehicleParameter("SpeedTargets/MPH/SpeedometerSpeedTargetLow", float(0.0));
    float maxSpeed = GetVehicleParameter("SpeedTargets/MPH/SpeedometerSpeedTargetHi", float(0.0));
    char buff[32];
...
//-------------------------------------------------------------------------------------------------
const string MazdaMachineTC::TestStepReverse(const string &value) {   // Log the entry and prompt the operator to accelerate in reverse
...
//-------------------------------------------------------------------------------------------------
string MazdaMachineTC::WaitToStart(void) {   // Log the entry and wait for the loss of zero speed
...
//-------------------------------------------------------------------------------------------------
const string MazdaMachineTC::TestStepAccelerateToSpeed(const string &value) {   // Log the entry and prompt the operator
