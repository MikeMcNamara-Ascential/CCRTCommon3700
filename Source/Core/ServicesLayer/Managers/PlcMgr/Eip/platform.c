...
BOOL platformGetTcpIpCfgData( TCPIP_CONFIG_DATA *psCfg)
{
	...
	DumpStr2(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "IP address of %s - %s", 
			 iface , inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) );
	getifaddrs(&ifaddr);
	for(i = 0, ifa = ifaddr; (ifa != NULL) && (i < iInterfaces); i++, ifa = ifa->ifa_next)
	{
		...
		{
			DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Checking address record for IP Address");
			if(platformGetInternetAddress(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) == psCfg->lIpAddr)
			{
				DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Found matching address record for IP Address %s == %08X",
			...
			{
				DumpStr0(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Address record did not match for IP Address");
			}
		}
	}

	//couldn't find IP address
	DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "Found IP Address: %s", i == iInterfaces ? "False" : "True");
	if(i == iInterfaces)
		return FALSE;
...
//		psCfg->lSubnetMask = platformGetInternetAddress(addr->sin_addr);
		psCfg->lSubnetMask = platformGetInternetAddress(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr);
		DumpStr2(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "\tNetmask: %08X [%s]", psCfg->lSubnetMask,
				 inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
	}
	else
	{
		DumpStr1(TRACE_LEVEL_USER, TRACE_TYPE_MSG, 0, 0, "ERROR:  Could not get netmask of %s", 
				 inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
	}
		...
		if((f = fopen("tmpfile","r")) != NULL)
		{
			...
			if(fgets( szTmp, sizeof(szTmp), f) != NULL)
			{
				...
				while(fgets( szTmp, sizeof(szTmp), f) != NULL)
				{
					...
					DumpStr2(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "Dest addr = %s [host ip: %s]", 
							 p, inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
					if(strcmp(p, inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr)) == 0)
					{
						p = strtok(NULL, "\n\r\t ");
						if(p == NULL)
							continue;
...
void platformSocketInitTargetData( void )
{
	...
	DumpStr1(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "Found %d interfaces to evaluate", iInterfaces);
	   ...
	   i < iInterfaces; i++, ifrp++)
	{
		...
		DumpStr2(TRACE_LEVEL_NOTICE, TRACE_TYPE_MSG, 0, 0, "socketInitTargetData evaluating interface: %s with address: %s", 
				 ifrp->ifr_name, inet_ntoa(sa->sin_addr));
		   ...
		   (ifrp->ifr_addr.sa_family == AF_INET6))
		{
			...
			DumpStr1(TRACE_LEVEL_NOTICE, TRACE_TYPE_MSG, 0, 0, "socketInitTargetData evaluating address %s", 
...
void platformSocketInitTargetData( void )
{
	...
	for(ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	{
		...
		if((strcmp(ifa->ifa_name, iface)==0) &&
		   (ifa->ifa_addr->sa_family == AF_LINK))
		{
			getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in), mac, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
			DumpStr3(TRACE_LEVEL_ERROR, TRACE_TYPE_MSG, 0, 0, "MAC of %s (family: %d) - %s", 
					 ifa->ifa_name, ifa->ifa_addr->sa_family, mac);
			//don't include "local" address
			DumpStr1(TRACE_LEVEL_NOTICE, TRACE_TYPE_MSG, 0, 0, "socketInitTargetData evaluating address %s", 
					 inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
			if(platformGetInternetAddress(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) != LOCAL_IP_ADDRESS)
			{
				galHostIPAddr[gnHostIPAddrCount] = platformGetInternetAddress(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr);
				gnHostIPAddrCount++;
			}
...
//				if(ioctl(sockfd, SIOCGIFHWADDR, ifrp) < 0)
				...
				for(ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
				{
					...
					if((strcmp(ifa->ifa_name, ifrp->ifr_name) == 0) &&
					   (ifa->ifa_addr->sa_family == AF_LINK))
					{
						getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in), mac, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
						p = strtok(mac, ":");
...
void platformInitLogFile(void)
{
	...
	for( i = 0; i < 1000; i++ )
	{
		sprintf( szFileName, "/home/CCRT/Rewrite/Logs/Communication/eip_debug%d.txt", i);
