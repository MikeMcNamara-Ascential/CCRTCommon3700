...
void SideSlipMonitor::Initialize(const XmlNode *document)
{
	...
	try
	{
		m_conversionFactor = BposReadInt(document->getChild("Setup/Parameters/SideSlipConversionFactor")->getValue().c_str());
	}
	catch(XmlException &excpt)
	{
		Log(LOG_ERRORS, "Could not find conversion factor, setting to 1.0 - %s", excpt.GetReason());
...
INT32 SideSlipMonitor::UpdateSideSlipValue()
{
	...
	if(BEP_STATUS_SUCCESS == status)
	{
		...
		if(BEP_STATUS_SUCCESS == status)
		{
			currentSideSlipValue = (float)BposReadInt(value.c_str()) * m_conversionFactor;
			if(currentSideSlipValue > m_currentMaxSideSlipValue)
			{
