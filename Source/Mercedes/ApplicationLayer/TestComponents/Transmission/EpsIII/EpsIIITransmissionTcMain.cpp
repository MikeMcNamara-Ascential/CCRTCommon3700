...
{	// command line processor
	...
	try
	{
		...
		if(clp.IsDebugOn())	printf("Creating The EPS-III Transmission Component\n");
		transmissionTest = new EpsIIITransmissionTc();
		// Check if we should perform the test
		if (transmissionTest != NULL)
		{	// Initialize the new test component
			if(clp.IsDebugOn())	printf("Initializing The EPS-III Transmission Component\n");
			transmissionTest->Initialize(clp.GetConfigFile());
			if(clp.IsDebugOn())	printf("EPS-III Transmission Component Running\n");
	...
	{
		printf("EPS-III Transmission -%s: XmlExcpetion: %s", clp.GetConfigFile().c_str(), XmlErr.what());
	}
	catch(BepException &BepErr)
	{
		printf("EPS-III Transmission -%s: BepExcpetion: %s", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("EPS-III Transmission -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}

    if(clp.IsDebugOn())
		printf("EPS-III Transmission (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
