//*************************************************************************
// FILE DESCRIPTION:
//  System monitor for Changan.  The system monitor will determine when
...
//===========================================================================
//*************************************************************************
#include "ChanganSystemMonitor.h"

//-------------------------------------------------------------------------------------------------
ChanganSystemMonitor::ChanganSystemMonitor() : SystemMonitor()
{   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
ChanganSystemMonitor::ChanganSystemMonitor(int argc, char *argv[]) : SystemMonitor(argc, argv)
{   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
ChanganSystemMonitor::~ChanganSystemMonitor()
{   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
void ChanganSystemMonitor::CheckTesting(ControlData *ctrl)
{   // Log the entry
    static bool testSelected = false;
    Log(LOG_FN_ENTRY, "ChanganSystemMonitor::CheckTesting() - Enter");
    ...
    {   //  If a test is in progress, there is nothing to do
        ...
        {   // Remove prompts and command test to start
            ...
            CommandNdbData(string(START_VEHICLE_TEST_DATA_TAG), true);
            testSelected = true;
        }
#if 0
    ...
    // Log the exit
    Log(LOG_FN_ENTRY, "ChanganSystemMonitor::CheckTesting() - Exit");
}

//-------------------------------------------------------------------------------------------------
const string ChanganSystemMonitor::Publish(const XmlNode *node)
{   // If this is the vehicle type, load the build data
    Log(LOG_FN_ENTRY, "ChanganSystemMonitor::Publish(%s, %s) - Enter", node->getName().c_str(), node->getValue().c_str());
    if(!node->getName().compare("VehicleTypeFromPLC"))
    {   
        CommandNdbData(string(READ_LATEST_BUILD_DATA_TAG), string("1")); 
    }
    Log(LOG_FN_ENTRY, "ChanganSystemMonitor::Publish(%s, %s) - Exit", node->getName().c_str(), node->getValue().c_str());
