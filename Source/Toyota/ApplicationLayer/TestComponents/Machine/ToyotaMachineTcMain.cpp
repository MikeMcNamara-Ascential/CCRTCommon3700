//******************************************************************************
// FILE DESCRIPTION:
// Toyota specific machine component
//
//==============================================================================
...
int main(int argc, char *argv[])
{
	...
	{
		printf("ToyotaMachineTC -%s: XmlExcpetion: %s", clp.GetConfigFile().c_str(), XmlErr.what());
	}
	catch(BepException &BepErr)
	{
		printf("ToyotaMachineTC -%s: BepExcpetion: %s", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("ToyotaMachineTC -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}

	delete component;

	if(clp.IsDebugOn())  printf("ToyotaMachineTC (%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
}
