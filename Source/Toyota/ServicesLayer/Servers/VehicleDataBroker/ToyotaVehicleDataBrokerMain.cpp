...
int main(int argc, char *argv[])
{
	...
	try
	{
		if(clp.IsDebugOn())	printf("Creating The ToyotaVehicleDataBroker\n");
		server = new ToyotaVehicleDataBroker();		// create a ToyotaVehicleDataBroker server
		// create, initialize, and process messages for the named data broker
		if(clp.IsDebugOn())	printf("Initializing The ToyotaVehicleDataBroker\n");
		server->Initialize(clp.GetConfigFile());
		if(clp.IsDebugOn())  printf("ToyotaVehicleDataBroker (%d, %s) Up - Listening\nRunning The ToyotaVehicleDataBroker\n",
                                    BposGetMyTaskId(), clp.GetConfigFile().c_str());
		server->Run();							// process until terminated
	}
	catch(BepException &BepErr)
	{
		printf("ToyotaVehicleDataBroker -%s: BepException: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
	}
	catch(...)
	{
		printf("ToyotaVehicleDataBroker -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
	}
	if(clp.IsDebugOn())	printf("Deleting The ToyotaVehicleDataBroker\n");
	if( NULL != server)	delete server;
	if(clp.IsDebugOn())  printf("ToyotaVehicleDataBroker (%d, %s): Terminating\n", 
                                BposGetMyTaskId(), clp.GetConfigFile().c_str());
}
