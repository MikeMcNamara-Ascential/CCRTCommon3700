...
//
// FILE DESCRIPTION:
//  Main loop for a JohnDeerePlantHostInbound.  
//  
//===========================================================================
...
/**
 * Main loop for the JohnDeerePlantHostInbound
 *
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        JohnDeerePlantHostInbound server;        // Server object
    ...
    {
        printf("JohnDeerePlantHostInbound -%s: Exception: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
    }
    catch(...)
    {
        printf("JohnDeerePlantHostInbound -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
    }
    if(clp.IsDebugOn()) printf("JohnDeerePlantHostInbound(%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
    return(0);
}
