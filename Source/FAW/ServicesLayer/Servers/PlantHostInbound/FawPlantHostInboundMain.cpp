...
//
// FILE DESCRIPTION:
//  Main loop for a FawPlantHostInbound.  
//  
//===========================================================================
...
/**
 * Main loop for the FawPlantHostInbound
 *
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        FawPlantHostInbound server;        // Server object
    ...
    {
        printf("FawPlantHostInbound -%s: Exception: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
    }
    catch(...)
    {
        printf("FawPlantHostInbound -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
    }
    if(clp.IsDebugOn()) printf("FawPlantHostInbound(%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
    return(0);
}
