...
//-----------------------------------------------------------------------------
const string GMLossCompensation::TestStepInitialize(void)
...
{   // Reset the status to test in progress
	...
	if(SystemReadBool("PLCReadyToTest"))
	{
		string result = GenericTC::Command(command);
		status = !result.compare(BEP_SUCCESS_RESPONSE) ? BEP_STATUS_SUCCESS : BEP_STATUS_FAILURE;
	}
	else
	{
		Log(LOG_ERRORS, "PLC Is not ready for test, not running loss compensation");
