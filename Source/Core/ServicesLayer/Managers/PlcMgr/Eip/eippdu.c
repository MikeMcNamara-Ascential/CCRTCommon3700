...
INT32  pduParse( UINT8* pPacketPtr, EPATH* pPDU, BOOL bPDUHeader, UINT32 nPDUFreeLength )
{
    ...
    UINT8            bExtendedSymbol, bExtendedSymbolType, bExtendedSymbolSize, bExtSymbolSize;
    UINT16           iDataSize;
    ETAG*            pPDUTag;
	...
	if (bPDUHeader)
	{
	    ...
	    nTotal = pdu_hdr.bSize * 2 + 2;                  /* PDU Length not including the PDU header */
		nParsed = PDU_HDR_SIZE;
	}
	else
	{
	    nTotal = nPDUFreeLength;                        /* No PDU header */
		nParsed = 0;
	}
    ...
    if ( nTotal == 0 )
        return nParsed;                    /* Just the header */
    ...
    {        
        ...
        switch( bSegmentType )                        /* Segment type determine the data length and structure that follows */
        {
            case PATH_SEGMENT:
            {
                ...
                else if ( bLinkAddressSize == 1 )    /* Destination slot number in a rack */
                {
            ...
            case SYMBOLIC_SEGMENT:
            {
                ...
                if ( bSymbolSize == 0 )
                {
                    ...
                    bExtendedSymbolType = (UINT8)(bExtendedSymbol & EXTENDED_SYM_TYPE_MASK);
