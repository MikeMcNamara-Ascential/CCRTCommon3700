...
int main(int argc, char *argv[])
{
	...
	try
	{
        ...
        object = new RwalBrakeTest<GenericABSModuleTemplate<SerialProtocolFilter> >();
		if(clp.IsDebugOn())	printf("Creating the RWAL Brake Component\n");

		if(clp.IsDebugOn())	printf("Initializing the RWAL Brake Component\n");
		object->Initialize(clp.GetConfigFile());

		if(clp.IsDebugOn())	printf("Running the RWAL Brake Component\n");
