...
//
// LOG:
//     $Log: /CMake Core 062909/Ford/Source/Core/CORE/ServicesLayer/Managers/PlcMgr/PlcEip.h $
// 
...
// $NoKeywords: $
//==============================================================================
#ifndef _PLCEIP_H_INCLUDED_
#define _PLCEIP_H_INCLUDED_
...
#include "EipProtocol.h"   // EIP Protocol Wrapper
...
typedef struct
{
    ...
    /**
     * Object used to interface to the EIP serial PLC
     */
    EipProtocol                *plc;
    ...
    /**
     * EIP source node ID for our system to use when sending EIP messages to a PLC
     */
    uint8_t                     eipSourceId;
...
/**
 * Allen Bradley Serial EIP protocol Board Object. Used to control an AB PLC board.
 *
 * @author Brian Meinke
 * @version 1.0
 * @since 12/11/2002
 */
class PlcEip : public IPlc
{
    friend class PlcEipTest;
...
public:
     ...
     * @return this
     */
    PlcEip& operator=( const PlcEip& rhs);
...
private:
     ...
     * via serial messages to and from the EIP PLC). Alos opens the logical
     ...
     */
    static void* EipCommThread( void *threadArg);
    ...
    /**
     * Argument to pass to the EIP serial comm thread
     */
    EipThreadArg_t              m_eipThreadArg;

    /**
     * Name of the logical port to communicate with the PLC on
     */
    std::string                 m_eipPortName;
