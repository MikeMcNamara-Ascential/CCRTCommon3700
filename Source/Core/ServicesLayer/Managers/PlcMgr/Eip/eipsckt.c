...
/* local functions */
BOOL socketEncapSend(SESSION* pSession, INT32 len, INT32* plenSent);
...
void socketPostListen(SOCKET* pWaitSocket, UINT32 lClaimedHostIPAddr, UINT16 iPort)
{
    ...
    if ( platformBind( lSocket, ( struct sockaddr * ) & sSrv_addr, sizeof( sSrv_addr ) ) == SOCKET_ERROR ) 
    {
         ...
         (*pWaitSocket) = (UINT32)INVALID_SOCKET;
         return;
    }
...
void socketCheckIncomingSession()
{
        ...
        pTcpIpConfig++)
    {
        ...
        lSocket = platformAccept(pTcpIpConfig->lPassiveWaitSocket,
                            ( struct sockaddr * ) &sCli_addr,
                            &lAddrLen ); 
        ...
        nStatus = platformSetSockOpt(    lSocket,
                                SOL_SOCKET,
                                SO_DONTLINGER,
                                (char*)&bLinger,
                                sizeof( bLinger ) );
		...
		gSystemStats.iNumTCPConnectionsUsed++;
		if (gSystemStats.iMaxTCPConnectionsUsed < (gpnSessions-gSessions))
			gSystemStats.iMaxTCPConnectionsUsed = (UINT16)(gpnSessions-gSessions);
#endif
    }
...
{    
    ...
    while( lBytesReceived < ENCAPH_SIZE )    /* The packet should be at least ENCAPH_SIZE bytes long */
    {                       
    ...
    /* Ignore No operation messages */   
    if ( iCommand == ENCAP_CMD_NOOP )
    {
        DumpStr3(TRACE_LEVEL_NOTICE, TRACE_TYPE_SESSION, pSession->lHostIPAddr, pSession->lClientIPAddr, 
			"socketEncapRecv received NOP command with %d bytes on socket 0x%x for session %d", lBytesRequested, pSession->lSocket, pSession-gSessions);
        return 0;
    }
...
BOOL socketEncapSend(SESSION* pSession, INT32 lLen, INT32* plenSent)
{   
    INT32        lSent = 0;
    INT32        lSockError;
...
INT32 socketClass1Recv( SOCKET lClass1Socket, UINT32 lHostIp )
{
    ...
    lBytesReceived = platformRecvFrom( lClass1Socket,
                                 (char*)gmsgBuf,
                                 MAX_PACKET_SIZE,
                                 0,
...
void socketGetBroadcasts(void)
{
        ...
        pTcpIpConfig++)
    {
        ...
        /* Try to read from the UDP socket into the gmsgBuf buffer. The socket is set to non-blocking, 
          so we'll know the results right away */
        lBytesReceived = platformRecvFrom(pTcpIpConfig->lBroadcastSocket,
                                  (char*)gmsgBuf,
                                  MAX_PACKET_SIZE,
                                  0,
                                  ( struct sockaddr * )&sClientAddr,
		...
		for(pTcpIpConfig2 = gsTcpIpInstAttr; 
			...
			pTcpIpConfig2++)
		{
			if (pTcpIpConfig2->InterfaceConfig.lIpAddr == 
					platformGetInternetAddress(sClientAddr.sin_addr))
			{
				bInternalMsg = TRUE;
				break;
			}
		...
		if (!bInternalMsg)
		{
			socketHandleBroadcasts(pTcpIpConfig, 
									&sClientAddr,
									gmsgBuf, lBytesReceived);
		}
    }
}
...
{
	INT32 len = ENCAPH_SIZE + psEncapHdr->iLength,
		  lSent;

    /* Convert the encapsulation header back */
	socketAdjustEncapHeader(psEncapHdr);

    lSent = platformSendTo( pTcpIpConfig->lBroadcastSocket, 
                       gmsgBuf,
                       len,
                       0,
...
{
	ENCAPH* pEncap = (ENCAPH*)gmsgBuf;
    INT32   lSent;
    struct sockaddr_in sClientAddr;

	memcpy(pEncap, encap, ENCAPH_SIZE);

	/* setup the broadcast */
	platformSetSockaddrIn( &sClientAddr, ENCAP_SERVER_PORT, INADDR_BROADCAST );
    ...
    if( lSent <= 0 )    
    {
		...
		lSent = platformGetLastSocketError(pTcpIpConfig->lBroadcastSocket);
		return FALSE;
	}
	...
	DumpBuf(TRACE_LEVEL_DATA, TRACE_TYPE_BCAST_OUT, pTcpIpConfig->InterfaceConfig.lIpAddr, 
        platformGetInternetAddress(sClientAddr.sin_addr),(char*)gmsgBuf, lSent);
...
INT32 socketClass1Send( CONNECTION* pConnection, INT32 len )
{
    ...
    if( lSent <= 0 )    
    {
        ...
        if( lSent < 0 && lSockError != EWOULDBLOCK )
        {
...
#ifdef CONNECTION_STATS
		gSystemStats.iNumFailedSentIOMessages++;
...
#ifdef CONNECTION_STATS
	gSystemStats.iNumSuccessfulSentIOMessages++;
#endif
