...
const std::string TestSequencer::SequenceTest(const std::string type)
{
   ...
   do
   {
         ...
         {   // walk through the phases until complete
            ...
            {   // Reset the in-cycle retest counter
               ...
               if (ResetPhase(incycleRetest))               // reset the phase in the drive curve
               {
                  ...
                  else Log(LOG_DEV_DATA, "No Foregound Test Sequenced For: %s\n", m_currentPhase->getName().c_str());
               }
               else Log(LOG_DEV_DATA, "Nothing To Sequence In: %s\n", m_currentPhase->getName().c_str());
            }
         }
         catch (BepException& e)
         {
      ...
      else if (GetStatus() == BEP_TESTING_STATUS) Log(LOG_DEV_DATA, "Background Objectives Already Complete\n");
      ...
      if (GetStatus() != BEP_TESTING_STATUS) UpdateCurrentStatus(GetStatus(), testStatus);
            ...
            if (commHandler.HandleRequest(message, this) == BEP_STATUS_SUCCESS)
            {
               if (chid > 0)    // if from a client (not a pulse)
               {
                  ...
                  {   // validate the mesage and update the drive curve
                     ...
                     if (testStatus == BEP_SOFTWAREFAIL_RESPONSE) SetStatus(BEP_SOFTWAREFAIL_STATUS);
                  }
                  else Log(LOG_ERRORS, "Unexpected Message: %s\n", message.c_str());

                  // check the test phase status
                  if ((status = TestPhaseStatus()) != BEP_TESTING_RESPONSE) done = true;
                  Log(LOG_DEV_DATA, "ProcessResults status: %s\n", status.c_str());
               }
            }
            else Log(LOG_ERRORS, "Wacked Message Received %s\n", message.c_str());
         }
         else Log(LOG_DEV_DATA, "ProcessResults Wakeup Event, Check %s Testing Status: %s\n",
      ...
      {   // wait for the response until the timeout time hits
         ...
         if (chid != -1)
         {
            ...
            if (commHandler.HandleRequest(message, this) == BEP_STATUS_SUCCESS)
            {
               if (chid > 0)    // if from a client (not a pulse)
               {
                  if (commHandler.GetRequestType(message) == "Write") objStatus = UpdateTestStatus(&commHandler); // validate mssg and update the drive curve
                  else Log(LOG_ERRORS, "Unexpected Message: %s\n", message.c_str());

                  // check the test status
                  UpdateCurrentStatus(objStatus, status);
               }
               else Log(LOG_DEV_DATA, "Waiting for the NumberOfObjectivesLeft %d == 0\n", m_backCompManager.NumberOfObjectivesLeft());
            }
            else Log(LOG_ERRORS, "Unknown Error Receiving Background Results: %s\n", message.c_str());
         }
         else Log(LOG_DEV_DATA, "ProcessBackgroundResults Timeout, Check For End Of Testing: %d\n", m_backCompManager.NumberOfObjectivesLeft());
      }
   }
   catch (XmlException& e)
   {
...
{   // check the message to determine if it is a results message
   ...
   if (value == "Foreground") status = UpdateForegroundObjectiveStatus(commHandler);
   else if (value == "Background") status = UpdateBackgroundObjectiveStatus(commHandler);
   else throw BepException("UpdateTestStatus Invalid Message Type: %s, %s", tag.c_str(), value.c_str());
   ...
   {   // if in the background, we must search all of the drive curve for the correct objective
      ...
      for (UINT32 ii = 0; (ii < m_driveCurve->getChildren().size()) && (!updateComplete); ii++)
      {
         ...
         {   // get the next background test phase (if it exists for the current phase)
            ...
            else Log(LOG_DEV_DATA, "Checked Phase: %s, Objective: %s\n",
      ...
      else UpdateCurrentStatus(value, m_backgroundObjectiveStatus);
   }
   catch (XmlException& e)
   {   // if the node does not exist thats ok, just dont do anything
...
{   // Order of results in importance:
   ...
   else Log(LOG_DEV_DATA, "Not Updating The Status: %s, %s\n", result.c_str(), overallResult.c_str());
...
const std::string TestSequencer::PromptRetest(void)
{
   ...
   {   // Command input server to TestResult state
      ...
      if (INPUT_SERVER_TESTRESULT_STATE == driverResponse) inputServerStatus = BEP_STATUS_SUCCESS;
      else BposSleep(100);
   ...
   {   // Clear the button tags before reading
      ...
      while ((timeout--) && !done && CanRetest(GetStatus()) && (StatusCheck() == BEP_STATUS_SUCCESS))
      {
         ...
         else
         {
            ...
            if (m_ndb->Read(tag, msgResponse, true) == BEP_STATUS_SUCCESS) m_ndb->GetNext(tag, driverResponse, msgResponse);
            else Log(LOG_ERRORS, "Unable to get the KeyPress information: %s\n", msgResponse.c_str());
         }
...
void TestSequencer::SetSequencerCommObj(IBepCommunication *sequencerComm)
{
   ...
   if (sequencerComm != NULL) m_sequencerComm = sequencerComm;
   else throw BepException("Null Sequencer Comm Passed To Sequencer\n");
}
