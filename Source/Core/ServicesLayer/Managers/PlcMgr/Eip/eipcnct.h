...
#ifdef CONNECTION_STATS
extern EtIPSystemStats gSystemStats;
#endif
...
extern INT32 connectionGetConnectedCount(void);                          /* Returns number of active connections */
...
extern CONNECTION* connectionGetFromSerialNumber(UINT16 iConnectionSerialNbr, UINT16 iVendorID, UINT32 lDeviceSerialNbr); /* Return an array index based on the connection Serial Number/vendor Id/device Serial Number combination */
...
extern void	connectNotifyNewRedundantMaster(CONNECTION* pConnection);
extern void	connectNotifyNoRedundantMaster(CONNECTION* pConnection);
extern void connectionConvertFromInternal(CONNECTION_CFG* pcfg, EtIPConnectionConfig* pConfig);
extern void connectionNotifyNetLEDChange();
