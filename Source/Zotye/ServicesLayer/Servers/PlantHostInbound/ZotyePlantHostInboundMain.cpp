...
//
// FILE DESCRIPTION:
//  Main loop for a ZotyePlantHostInbound.  
//  
//===========================================================================
...
/**
 * Main loop for the ZotyePlantHostInbound
 *
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ZotyePlantHostInbound server;        // Server object
    ...
    {
        printf("ZotyePlantHostInbound -%s: Exception: %s\n", clp.GetConfigFile().c_str(), BepErr.what());
    }
    catch(...)
    {
        printf("ZotyePlantHostInbound -%s: Unknown Exception\n", clp.GetConfigFile().c_str());
    }
    if(clp.IsDebugOn()) printf("ZotyePlantHostInbound(%d, %s): Terminating\n", BposGetMyTaskId(), clp.GetConfigFile().c_str());
    return(0);
}
