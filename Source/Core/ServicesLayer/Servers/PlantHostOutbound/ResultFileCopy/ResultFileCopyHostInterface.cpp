//******************************************************************************
// Description:
//   ResultFileCopy Host Interface base lass to handle copying the test result file to the
//   special directory so it can be processed and placed on the ResultFileCopy network.
//
//===========================================================================
...
//-------------------------------------------------------------------------------------------------
ResultFileCopyHostInterface::ResultFileCopyHostInterface() : HostInterface()
{   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
ResultFileCopyHostInterface::~ResultFileCopyHostInterface()
{   // Nothing special to do here
}

//-------------------------------------------------------------------------------------------------
void ResultFileCopyHostInterface::Initialize(int argc, char *argv[])
{
    HostInterface::Initialize(argc, argv);
}

//-------------------------------------------------------------------------------------------------
void ResultFileCopyHostInterface::Initialize(const string &fileName)
{
    HostInterface::Initialize(fileName);
}

//-------------------------------------------------------------------------------------------------
void ResultFileCopyHostInterface::Initialize(const XmlNode *document)
...
//-------------------------------------------------------------------------------------------------
void ResultFileCopyHostInterface::Run(volatile bool *terminateFlag)
{
    bool notReady = false;
    bool ready = true;
    ...
    {   // Wait for a result to process
        ...
        {   // Set a flag indicating we cannot process any results
            ...
            if(!fileName.empty())
            {
                string command(string("cp -c ") + fileName + string(" ") + GetDataTag("ResultFileCopyResultDirectory"));
                if(system(command.c_str()) == -1)
                {
                    Log(LOG_ERRORS, "Error copying %s to %s", 
                        fileName.c_str(), GetDataTag("ResultFileCopyResultDirectory").c_str());
                }
                else
                {
...
//-------------------------------------------------------------------------------------------------
const string ResultFileCopyHostInterface::Write(const XmlNode *node)
{
    string result(BEP_ERROR_RESPONSE);
    Log(LOG_FN_ENTRY, "ResultFileCopyHostInterface::Write(%s) - Enter", node->getName().c_str());
    ...
    Log(LOG_FN_ENTRY, "ResultFileCopyHostInterface::Write(%s) - Exit: %s", node->getName().c_str(), result.c_str());
    return result;
}

//-------------------------------------------------------------------------------------------------
string ResultFileCopyHostInterface::PopTestResult(void)
{
    string fileName = "";
...
//-------------------------------------------------------------------------------------------------
void ResultFileCopyHostInterface::PushTestResult(const string &fileName)
