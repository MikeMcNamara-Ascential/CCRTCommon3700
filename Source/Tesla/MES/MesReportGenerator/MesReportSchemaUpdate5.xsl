		...
		<xsl:element name="DtcCodes">
			<xsl:for-each select="//TestResult/DTC/Brake/child::*">
				<xsl:element name="Dtc">
					<xsl:element name="DtcCode">0x<xsl:value-of select="./@DTCCode" /></xsl:element>
					<xsl:element name="DtcDescription"><xsl:value-of select="." /></xsl:element>
				</xsl:element>
			</xsl:for-each>
		</xsl:element>
	</xsl:template>
