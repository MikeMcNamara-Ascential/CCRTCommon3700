...
** eipCnLst.c
**
** COPYRIGHT (c) 2011 Pyramid Solutions, Inc.
**
*****************************************************************************
*****************************************************************************
**
...
BOOL connListResetCnxnTimer(CONNECTION *pConn, UINT32 eType, UINT32 ulTime)
{
    ...
    else
    {
        ...
        else
        {
            ...
            connListSetCnxnTimerReference(pConn, eType, pEntry);    /* set reference in connection */
        }        
    }
...
void connListMoveCnxnListEntries(CONNECTION *pConn)
{
         ...
         */
    if (pConn->pPollListEntry)  ((CNXN_POLL_LIST_ENTRY *)(pConn->pPollListEntry))->pConn    = pConn;
    if (pConn->pConfigTimer)    ((CNXN_TIMER_LIST_ENTRY *)(pConn->pConfigTimer))->pConn     = pConn;
    if (pConn->pDelayTimer)     ((CNXN_TIMER_LIST_ENTRY *)(pConn->pDelayTimer))->pConn      = pConn;
    if (pConn->pInhibitTimer)   ((CNXN_TIMER_LIST_ENTRY *)(pConn->pInhibitTimer))->pConn    = pConn;
    if (pConn->pProduceTimer)   ((CNXN_TIMER_LIST_ENTRY *)(pConn->pProduceTimer))->pConn    = pConn;
    if (pConn->pTimeoutTimer)   ((CNXN_TIMER_LIST_ENTRY *)(pConn->pTimeoutTimer))->pConn    = pConn;
#ifdef CONNECTION_STATS
    if (pConn->pStatsTimer)     ((CNXN_TIMER_LIST_ENTRY *)(pConn->pStatsTimer))->pConn      = pConn;
#endif
}
...
static CNXN_POLL_LIST_ENTRY *connListGetPollEntryFromPool()
{
    ...
    if (psPollPool) /* pool is not empty */
    {
        ...
        pEntry = psPollPool;
        LLRemove((GENERAL_LIST_ENTRY *)psPollPool, 
                (GENERAL_LIST_ENTRY **)&psPollPool, NULL);
    }
...
static void connListReturnPollEntryToPool(CNXN_POLL_LIST_ENTRY *pEntry)
{
    ...
    LLInsert((GENERAL_LIST_ENTRY *)pEntry, NULL, 
            (GENERAL_LIST_ENTRY **)&psPollPool, NULL);
}
    ...
    else
    {
        if (psList->psCenter)
        {
            ...
            if (IS_TICK_GREATER_OR_EQUAL(pEntry->ulExpTime, psList->psCenter->ulExpTime))
            {
                ...
                if ((psList->psTail->ulExpTime - pEntry->ulExpTime) < (pEntry->ulExpTime - psList->psCenter->ulExpTime))
                {
                    pLoc = psList->psTail;      /* tail towards center */
                    walkBackwards = TRUE;
                }
                    /* it's closer to the center */
                else
                {
                    pLoc = psList->psCenter;    /* center towards tail */
                    walkBackwards = FALSE;
                }
            }
            ...
            else
            {
                ...
                if ((psList->psCenter->ulExpTime - pEntry->ulExpTime) < (pEntry->ulExpTime - psList->psHead->ulExpTime))
                {
                    pLoc = psList->psCenter;    /* center towards head */
                    walkBackwards = TRUE;
                }
                    /* it's closer to the head */
                else
                {
                    pLoc = psList->psHead;      /* head towards center */
                    walkBackwards = FALSE;
                }
            }
        }
