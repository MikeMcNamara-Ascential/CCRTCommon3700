...
namespace RomessSteeringwheelInclinometerInterface
{
	...
	internal sealed class HIDDLLInterface
	{
		...
		private const decimal WM_HID_EVENT = 32968;
        ...
        public static bool ConnectToHID(object targetForm, int targetHandle)
		{
            FWinHandle = targetHandle;
            int num = 0 - (hidConnect(FWinHandle) ? 1 : 0);
		...
		{
			if (decimal.Compare(new decimal(pMsg), new decimal(32968L)) == 0)
			{
				switch (wParam)
				{
				case 1:
